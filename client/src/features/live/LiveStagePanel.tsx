import { Check, Crown, Grid2X2, MicOff, Plus, UserRound, Users, X } from "lucide-react";
...
type Props = {
  ...
  centerParticipantId?: number | null;
  onRequestStage: () => void;
  onAccept: (requestId: string) => void;
  onReject: (requestId: string) => void;
  onRemove: (userId: number) => void;
  onMute: (userId: number, muted: boolean) => void;
  onSetCenter?: (userId: number) => void;
};

export function LiveStagePanel({ isHost, participants, requests, maxParticipants, myRequestPending, centerParticipantId, onRequestStage, onAccept, onReject, onRemove, onMute, onSetCenter }: Props) {
  const stage = participants.filter((p) => p.role === "host" || p.role === "guest" || p.role === "admin");
  return (
    <section className="absolute left-3 right-3 top-24 z-20 pointer-events-auto max-h-[42vh] overflow-y-auto">
      <div className="rounded-2xl bg-black/65 backdrop-blur-md border border-white/10 p-3">
        ...
        {isHost && stage.length > 1 && <p className="text-[11px] text-white/55 mb-2">Choisis qui doit être au centre. Les autres invités restent répartis sur les côtés.</p>}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">{stage.map((p) => <div key={p.userId} className={`rounded-xl p-2 min-h-16 border ${centerParticipantId === p.userId ? "border-yellow-400/70 bg-yellow-400/10" : "border-transparent bg-white/10"}`}><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-white/15 flex items-center justify-center"><UserRound size={16} /></div><div className="min-w-0 flex-1"><p className="text-xs font-semibold truncate">{p.username}</p><p className="text-[10px] text-white/60">{p.role === "host" ? "Hôte" : p.role === "admin" ? "Admin" : "Invité"}</p></div>{centerParticipantId === p.userId && <Crown size={13} className="text-yellow-300" />}{p.isMuted && <MicOff size={13} />}</div>{isHost && p.role !== "host" && <div className="flex gap-1 mt-2"><button type="button" onClick={() => onSetCenter?.(p.userId)} className="flex-1 rounded-lg bg-yellow-400/15 text-yellow-200 py-1 text-[10px]">Mettre au centre</button><button type="button" onClick={() => onMute(p.userId, !p.isMuted)} className="rounded-lg bg-white/10 px-2 py-1 text-[10px]">{p.isMuted ? "Micro" : "Muet"}</button><button type="button" onClick={() => onRemove(p.userId)} className="rounded-lg bg-red-500/80 px-2 py-1 text-[10px]">Retirer</button></div>}</div>)}</div>
