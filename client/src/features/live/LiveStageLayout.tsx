...
export function LiveStageLayout({ layout, participants, maxParticipants, renderVideo, centerParticipantId }: Props) {
  // maxParticipants means guest places; slot 0 is the host, so the visual stage has +1 place.
  ...
  if (layout === "host-center") {
    ...
    // Keep everyone except the selected center in the side columns. When the host is center,
    // this produces exactly 10 left + 10 right for 20 guests; for 23 guests it becomes 12/11.
    const side = slots.filter((p): p is Participant => !!p && p.userId !== centerId);
    const sideCapacity = guestCapacity;
    const splitLarge = guestCapacity >= 20;
    const leftCount = splitLarge ? Math.ceil(sideCapacity / 2) : sideCapacity;
    const rightCount = splitLarge ? sideCapacity - leftCount : 0;
