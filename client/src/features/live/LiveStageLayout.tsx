...
export function LiveStageLayout({ layout, participants, maxParticipants, renderVideo, centerParticipantId }: Props) {
  ...
  const spotlightMain = centerParticipantId ? stage.find((p) => p.userId === centerParticipantId) || slots[0] : slots[0]; const spotlightSide = slots.filter((p) => p && p.userId !== spotlightMain?.userId); return <div className="absolute inset-0 p-2">{mainVideo(spotlightMain, renderVideo, emptySlot)}<div className="absolute bottom-24 right-3 w-28 max-h-[55vh] overflow-y-auto space-y-2">{spotlightSide.map((p, i) => slot(p, i + 1, true))}</div></div>;
}
