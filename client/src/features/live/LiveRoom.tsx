...
export function LiveRoom() {
  ...
  useEffect(() => {
    ...
    const setup = async () => { try {
      ...
      socket.on("connect", () => { socket.emit("live:join", { sessionId, userId: me.id, username: me.name || "Utilisateur", role: isHost ? "host" : "viewer" }); if (isHost) socket.emit("live:layout", { sessionId, layout, centerParticipantId }); });
      socket.on("live:layout", ({ layout: nextLayout, centerParticipantId: nextCenter }) => { if (typeof nextLayout === "string" && isLiveLayoutId(nextLayout)) setLayout(nextLayout); if (Number.isInteger(nextCenter)) setCenterParticipantId(Number(nextCenter)); });
      ...
      socket.on("live:user-left", ({ socketId, userId }) => { peersRef.current.get(socketId)?.close(); peersRef.current.delete(socketId); peerUsersRef.current.delete(socketId); if (Number.isInteger(userId)) { remoteStreamsRef.current.delete(Number(userId)); setMediaRevision((v) => v + 1); setCenterParticipantId((current) => current === Number(userId) ? session.hostId : current); } });
  ...
  const setCenterParticipant = (userId: number) => { if (!isHost) return; setCenterParticipantId(userId); socketRef.current?.emit("live:layout", { sessionId, layout, centerParticipantId: userId }); };
