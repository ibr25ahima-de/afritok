...
function StreamVideo({ stream, muted = false, label, videoOff = false, className = "" }: StreamVideoProps) {
  ...
  return <div className={`relative w-full h-full overflow-hidden rounded-2xl bg-gray-950 ${className}`}><video ref={ref} autoPlay muted={muted} playsInline className="w-full h-full object-cover" />{(videoOff || !stream) && <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 text-center bg-gray-950"><CameraOff size={38} className="mb-2" /><span className="text-sm">{videoOff ? "Caméra désactivée" : label}</span></div>}</div>;
}
...
export function LiveRoom() {
  ...
  return <div className="h-[100dvh] bg-black text-white overflow-hidden relative">
    <LiveStageLayout layout={layout} participants={participants} maxParticipants={Number(session.maxParticipants || 5)} renderVideo={renderStageVideo} />
