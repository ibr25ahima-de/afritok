    ...
    <main className="min-h-screen bg-gray-950 text-white px-5 py-8 flex items-center justify-center">
      <div className="w-full max-w-lg rounded-3xl bg-gray-900 border border-white/10 p-6 shadow-2xl">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-12 h-12 rounded-2xl bg-red-500/15 text-red-400 flex items-center justify-center"><Radio /></div>
          <div><h1 className="text-2xl font-bold">Passer en Live</h1><p className="text-sm text-gray-400">Choisis le thème et le nombre de personnes sur scène.</p></div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {LIVE_STAGE_CAPACITIES.map((value) => <button key={value} type="button" onClick={() => onCapacityChange(value)} className={`rounded-xl py-2.5 font-bold border ${capacity === value ? "bg-red-500 border-red-400" : "bg-gray-950 border-white/10 text-gray-300"}`}>{value}</button>)}
        </div>
