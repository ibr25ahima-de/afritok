...
/**
 * Real-time skin retouch pass. It keeps eyes, lips and nose protected while
 * smoothing the surrounding skin, which is the important difference from a
 * global CSS blur/filter.
 */
function retouchSkin(ctx: CanvasRenderingContext2D, l: NormalizedLandmark[], w: number, h: number, amount: number) {
  ...
  if (!t) return;
  t.filter = `blur(${(3 + amount * 5.5).toFixed(1)}px) saturate(${(0.98 + amount * 0.04).toFixed(2)}) brightness(${(1.002 + amount * 0.012).toFixed(3)})`;
  ...
  ctx.globalAlpha = 0.58 + amount * 0.34;
...
function tone(ctx: CanvasRenderingContext2D, l: NormalizedLandmark[], w: number, h: number, amount: number) {
  ...
  g.addColorStop(0, `rgba(255,255,255,${0.045 * amount})`);
  g.addColorStop(0.65, `rgba(255,255,255,${0.018 * amount})`);
...
function localScale(ctx: CanvasRenderingContext2D, cx: number, cy: number, rx: number, ry: number, scale: number) {
  ...
  const c = crop.getContext("2d"); if (!c) return;
