...
const PACKAGES = {
  ...
  image: { label: "Image", icon: Image, prices: { 7: 10000, 14: 18000, 21: 25000, 30: 30000 } },
...
export default function Advertising() {
  ...
  const handleSubmit = async () => {
    ...
    if ((adType === "image" || adType === "video") && !mediaUrl.trim()) { setMessage(`Ajoutez l'URL de votre ${adType === "image" ? "image" : "vidéo"}.`); return; }
    ...
    try {
      ...
      setMessage(payment.success ? "Paiement lancé. La campagne sera activée après confirmation réelle du paiement." : "Campagne enregistrée. Le paiement est en attente : elle ne sera pas diffusée avant confirmation réelle du paiement.");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-10">
      ...
      <main className="max-w-xl mx-auto px-4 py-6 space-y-6">
        <section className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4"><h2 className="font-bold text-lg">Choisissez votre formule</h2><p className="text-sm text-gray-300 mt-1">Le prix affiché correspond à toute la durée de diffusion.</p></section>
        <div className="grid grid-cols-3 gap-2">
          {(Object.keys(PACKAGES) as AdType[]).map((type) => { const item = PACKAGES[type]; const Icon = item.icon; return <button key={type} onClick={() => setAdType(type)} className={`rounded-2xl p-4 border text-center ${adType === type ? "border-amber-400 bg-amber-400/15" : "border-white/10 bg-white/5"}`}><Icon className="mx-auto mb-2" size={24} /><span className="font-bold">{item.label}</span></button>; })}
        </div>
        <div className="grid grid-cols-2 gap-2">
          {([7,14,21,30] as Duration[]).map((days) => <button key={days} onClick={() => setDuration(days)} className={`rounded-xl border p-3 text-left ${duration === days ? "border-amber-400 bg-amber-400/15" : "border-white/10 bg-white/5"}`}><div className="font-bold">{days === 30 ? "1 mois" : `${days / 7} semaine${days > 7 ? "s" : ""}`}</div><div className="text-amber-400 font-black mt-1">{formatXof(PACKAGES[adType].prices[days])}</div></button>)}
        </div>
        <section className="space-y-3">
          ...
          {adType === "text" ? <textarea value={textContent} onChange={(e) => setTextContent(e.target.value)} placeholder="Écrivez votre publicité..." rows={5} className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-amber-400" /> : <input value={mediaUrl} onChange={(e) => setMediaUrl(e.target.value)} placeholder={`URL de votre ${adType === "image" ? "photo" : "vidéo"}`} className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-amber-400" />}
        ...
        <div className="rounded-2xl bg-amber-400 text-black p-5"><div className="flex justify-between items-center"><span className="font-bold">Total</span><span className="text-2xl font-black">{formatXof(price)}</span></div><button disabled={busy} onClick={handleSubmit} className="mt-4 w-full rounded-xl bg-black text-white py-4 font-black disabled:opacity-50">{busy ? "Traitement..." : "Envoyer et payer"}</button></div>
        ...
        <p className="text-xs text-gray-500 text-center">Une publicité n'est diffusée qu'après confirmation réelle du paiement et selon les règles de diffusion AfriTok.</p>
      </main>
    </div>
  );
}
