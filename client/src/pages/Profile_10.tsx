        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end">
          <div className="w-full bg-gray-900 rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-white">Monétisation</h2>
              <button
            ...
            <div className="bg-gradient-to-r from-red-500 to-pink-500 rounded-lg p-6 mb-6">
              <p className="text-gray-200 text-sm mb-1">Revenus totaux</p>
              <h3 className="text-4xl font-bold text-white mb-2">${totalEarnings.toFixed(2)}</h3>
              <p className="text-gray-100 text-xs">Mis à jour en temps réel</p>
            </div>

            {/* MONETIZATION PROGRESS BAR */}
            <MonetizationProgressBar 
              followers={followerCount?.followers || 0}
                ...
                <div key={source.id} className="bg-gray-800 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <p className="text-white font-bold">
                      ${(earningsBySource[source.id]?.total || 0).toFixed(2)}
                    </p>
                  </div>
                  <p className="text-gray-400 text-xs mb-3">
                    {earningsBySource[source.id]?.count || 0} transactions
                  </p>
                  <div className="space-y-1">
                    {source.conditions.map((condition, idx) => (
                      <p key={idx} className="text-gray-400 text-xs">• {condition}</p>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <button
              ...
              className="w-full mt-6 bg-red-500 hover:bg-red-600 text-white font-bold py-3 rounded-lg transition"
            >
              💸 Retirer {totalEarnings.toFixed(2)} USD
            </button>
          </div>
        </div>
      )}
