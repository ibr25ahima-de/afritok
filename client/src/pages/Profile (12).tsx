...
export default function Profile() {
  ...
  const [earnings, setEarnings] = useState<{ total: number; available: number; pending: number } | null>(null);
  ...
  const fetchEarnings = async () => {
    ...
    try {
      // ✅ 1. URL API (CORRIGÉ via ENV)
      const API_URL = process.env.EXPO_PUBLIC_API_URL;
      const res = await fetch(`${API_URL}/api/user/earnings`, {
        headers: {
  ...
  const withdrawMutation = trpc.instantWithdrawal.create.useMutation();

  // ✅ 4. Protection bouton retrait (CORRIGÉ)
            ...
            <div className="flex justify-around py-4 border-t border-gray-800">
              <div className="text-center">
                <p className="text-xl font-bold text-green-400">
                  {/* ✅ 3. Crash silencieux côté frontend (SAFE) */}
          ...
          <div className="w-full bg-gray-900 rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div className="bg-black border border-gray-800 rounded-xl p-4">
                <p className="text-gray-400 text-sm">Total gagné</p>
              ...
              <div>
                <h3 className="text-white font-bold mb-3">🚀 Comment gagner de l’argent</h3>
                <p className="text-sm text-gray-300">
                  💰 Chaque action te fait gagner de l’argent.<br />
                  Plus tu es actif, plus tu gagnes.<br />
                  Les créateurs gagnent encore plus avec leurs vidéos.
                </p>
                <p className="text-sm text-green-400 mt-2">
                  🚀 Astuce : Sois actif tous les jours pour maximiser tes gains
                </p>
              </div>

              <div>
                <h3 className="text-white font-bold mb-3">🎬 Gains créateur</h3>
                <p className="text-sm text-gray-300">
                  Chaque fois que quelqu’un regarde ta vidéo, tu gagnes de l’argent.
                </p>
              </div>

              <div>
                <h3 className="text-white font-bold mb-3">💸 Retrait facile et local</h3>
                <p className="text-sm text-gray-300">
                  Tu n'as pas besoin de carte bancaire ! 💳<br />
                  Tu peux retirer ton argent directement via ton moyen de retrait local (Orange Money, MTN, Wave, etc.) selon ton pays.<br /><br />
                  ✅ Minimum : 1000 CFA<br />
                  🚀 Paiement rapide sous 24h
                </p>
              </div>
            </div>
          </div>
