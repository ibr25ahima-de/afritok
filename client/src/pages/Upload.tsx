...
export default function Upload() {
  ...
  if (step === "edit") {
    ...
    return <div className="h-[100dvh] min-h-0 w-full max-w-full bg-black text-white overflow-hidden overflow-x-hidden relative">
        ...
        {tool === "text" && (
          <input
            ...
            style={{
              ...
              fontSize: Math.max(16, overlaySize), color: overlayColor, fontWeight: 700,
              textAlign: "center", textShadow: "0 2px 6px #000, 0 0 2px #000",
              background: "transparent", border: "none", outline: "none",
              width: "85%", maxWidth: "85%", zIndex: 25, caretColor: overlayColor,
      ...
      {showFilters && <div className="absolute inset-0 z-50 bg-black/95 p-4 overflow-y-auto overscroll-contain"><div className="flex justify-between items-center mb-5"><h2 className="font-bold">Filtres</h2><button onClick={() => setShowFilters(false)}><X /></button></div><div className="flex gap-3 overflow-x-auto pb-4">{QUICK_FILTERS.map((filter) => <button key={filter.id} onClick={() => { setEditFilter(filter); setShowFilters(false); }} className="min-w-[70px] text-center"><div className="h-14 rounded-xl bg-white/10" style={{ filter: filter.cssFilter }} /><span className="text-[10px]">{filter.name}</span></button>)}</div><FilterLibrary onFilterSelect={(filter: Filter) => { setEditFilter({ id: filter.id, cssFilter: filter.cssFilter || "none" }); setShowFilters(false); }} selectedFilters={editFilter ? [editFilter.id] : []} onFilterRemove={() => setEditFilter(null)} /></div>}
...
function InlineTextPanel({ value, setValue, size, setSize, color, setColor, onAdd, items, onDelete, onDone }: { value: string; setValue: (v: string) => void; size: number; setSize: (v: number) => void; color: string; setColor: (v: string) => void; onAdd: () => void; items: Overlay[]; onDelete: (id: string) => void; onDone: () => void }) {
  return <div className="space-y-3">
    <p className="text-xs text-white/50 text-center">Tape directement sur la vidéo, au centre.</p>
    <div className="flex items-center gap-4">
      <div className="flex-1"><div className="flex justify-between text-xs text-white/60"><span>Taille</span><span>{size}px</span></div><input type="range" min="16" max="72" value={size} onChange={(e) => setSize(Number(e.target.value))} className="w-full" /></div>
      <input type="color" value={color} onChange={(e) => setColor(e.target.value)} className="h-9 w-12 bg-transparent" aria-label="Couleur du texte" />
    </div>
    <div className="flex flex-wrap gap-2">{items.map((item) => <div key={item.id} className="flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5"><span className="max-w-40 truncate text-sm">{item.text}</span><button onClick={() => onDelete(item.id)} aria-label="Supprimer"><Trash2 size={14} /></button></div>)}</div>
    <button onClick={() => { onAdd(); onDone(); }} className="w-full py-2.5 rounded-xl bg-red-500 font-bold">Valider</button>
  </div>;
}
