...
export default function Upload() {
  ...
  if (step === "edit") return (
    <div className="h-screen bg-black text-white relative flex flex-col">
      ...
      {/* Simulation des boutons latéraux comme sur la capture */}
      <div className="absolute right-4 top-20 flex flex-col gap-6 z-20">
        <div className="flex flex-col items-center gap-1"><div className="p-2 bg-black/20 rounded-full"><RotateCw size={24}/></div></div>
        <div className="flex flex-col items-center gap-1"><div className="p-2 bg-black/20 rounded-full"><X size={24}/></div></div>
        <div className="flex flex-col items-center gap-1 text-sm font-bold">Aa</div>
      </div>

      <div className="mt-auto p-4 flex gap-3 z-20">
        <button onClick={() => { setFile(null); setStep("capture"); }} className="flex-1 bg-white/20 py-3 rounded-full font-bold flex items-center justify-center gap-2">
          <div className="w-6 h-6 rounded-full bg-blue-400 overflow-hidden"><img src={user?.image || ""} alt="" className="w-full h-full object-cover"/></div>
          Ta Story
        </button>
      ...
      <div className="flex-1 p-5 flex flex-col gap-5">
        <div className="flex gap-4">
          <textarea 
            ...
            className="flex-1 bg-transparent resize-none focus:outline-none text-lg h-32" 
          />
          <div className="w-24 aspect-[9/16] bg-neutral-900 rounded-lg overflow-hidden shrink-0">
            {file?.type.startsWith("image/") ? (
              <img src={previewUrl!} className="w-full h-full object-cover" alt="preview" />
            ) : (
