...
export default function Upload() {
  ...
  const faceCanvasRef = useRef<HTMLCanvasElement>(null); // Étape 2
  ...
  const [eyeEnhance, setEyeEnhance] = useState(50); // Étape 4
      ...
      if (ctx && video.readyState === 4) {
        ...
        if (results.faceLandmarks) {
          for (const landmarks of results.faceLandmarks) {
            // Dessiner les points (Yeux, Nez, Bouche)
  ...
  useEffect(() => {
    if (step === "capture") {
      const animationId = requestAnimationFrame(detectFace);
  ...
  if (step === "capture") return (
    <div className="h-screen bg-black text-white relative overflow-hidden flex flex-col">
      ...
      />
      {/* Canvas superposé pour MediaPipe (Étape 2) */}
      <canvas
        ref={faceCanvasRef}
      ...
      {/* Drawer Beauté mis à jour (Étape 4) */}
      {activeDrawer === "beauty" && (
        <Drawer title="Beauté" onClose={() => setActiveDrawer("none")}>
          <div className="space-y-6 py-4">
            {[
              ...
              { label: "Agrandir yeux", value: eyeEnhance, setter: setEyeEnhance }
            ].map(item => (
