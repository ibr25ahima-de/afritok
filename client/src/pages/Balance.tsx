...
export default function Balance() {
  ...
  const totalEarnings = earningsQuery.data
    ? (earningsQuery.data as Earning[]).reduce((sum, e) => sum + parseFloat(e.amount), 0)
    : 0;
    ...
    <div className="min-h-screen bg-black text-white pb-20">
      ...
      <div className="px-4 py-6 space-y-6">
            ...
            withdrawalsQuery.data.map((withdrawal: any) => (
              <div key={withdrawal.id} className="bg-gray-900/50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white font-semibold">{withdrawal.provider}</p>
        ...
        <div className="space-y-3">
          ...
          {earningsQuery.data && (earningsQuery.data as Earning[]).length > 0 ? (
            (earningsQuery.data as Earning[]).slice(0, 10).map((earning) => (
