    ...
    <div className="min-h-screen bg-black text-white pb-20">
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end">
          <div className="w-full bg-gray-900 rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-black/50 rounded-lg p-4 mb-6">
              ...
              <p className="text-4xl font-bold text-white mb-2">{profile?.currency || 'USD'} {totalEarnings.toFixed(2)}</p>
            ...
            {isOwnProfile && (
              <div className="mb-6">
                <MonetizationProgressBar
                  conditions={[
                    { id: 'followers', label: 'Abonnés', current: followerCount?.followers || 0, required: 100, unit: 'abonnés' },
                    { id: 'views', label: 'Vues (30j)', current: videos.reduce((sum, v) => sum + (v.views || 0), 0), required: 10000, unit: 'vues' },
                    { id: 'videos', label: 'Vidéos', current: videos.length, required: 3, unit: 'vidéos' },
                  ]}
                  isEligible={false}
                />
              </div>
            ...
            <div className="space-y-4">
              {revenueSources.map((source) => {
                ...
                return (
                  <div key={source.id} className="bg-black/50 rounded-lg p-4 hover:bg-black/70 transition">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          {count > 0 && (
                            <p className="text-gray-500 text-xs mt-1">{count} transaction{count > 1 ? 's' : ''}</p>
                          )}
                        </div>
                      </div>
                      <p className="text-2xl font-bold text-white">{profile?.currency || 'USD'} {amount.toFixed(2)}</p>
                    </div>

                    {/* CONDITIONS */}
                    <div className="bg-black/30 rounded p-3 mt-3">
                      <p className="text-gray-300 text-xs font-semibold mb-2">Conditions :</p>
                      <ul className="text-gray-400 text-xs space-y-1">
                        {source.conditions.map((condition, idx) => (
                          <li key={idx} className="flex items-center gap-2">
                            <span className={source.id === 'auto_earn' ? 'text-green-400' : 'text-blue-400'}>•</span>
                            {condition}
                          </li>
                        ))}
                      </ul>
            ...
            {/* WITHDRAWAL BUTTON */}
            {isOwnProfile && totalEarnings > 0 && (
              <button
                onClick={() => {
                  setShowMonetization(false);
                  navigate('/instant-withdraw');
                }}
                className="w-full mt-6 bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg font-bold transition flex items-center justify-center gap-2"
              >
                💸 Retirer {(profile?.currency || 'USD')} {totalEarnings.toFixed(2)}
              </button>
            )}

            <button
              onClick={() => setShowMonetization(false)}
              className="w-full mt-3 bg-red-500 hover:bg-red-600 text-white py-3 rounded-lg font-bold transition"
            >
              Fermer
            </button>
          </div>
      ...
      {showFollowers && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end">
          <div className="w-full bg-gray-900 rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-white">Abonnés ({followerCount?.followers || 0})</h2>
              <button
            ...
            <p className="text-gray-400">Liste des abonnés à venir...</p>
            <button
              onClick={() => setShowFollowers(false)}
              className="w-full mt-6 bg-red-500 hover:bg-red-600 text-white py-3 rounded-lg font-bold transition"
            >
              Fermer
            </button>
          </div>
      ...
      {showFollowing && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end">
          <div className="w-full bg-gray-900 rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-white">Suivis ({followerCount?.following || 0})</h2>
              <button
            ...
            <p className="text-gray-400">Liste des comptes suivis à venir...</p>
            <button
              onClick={() => setShowFollowing(false)}
              className="w-full mt-6 bg-red-500 hover:bg-red-600 text-white py-3 rounded-lg font-bold transition"
            >
      ...
      {showLikes && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end">
          <div className="w-full bg-gray-900 rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-gray-400">Détail des likes à venir...</p>
            <button
              onClick={() => setShowLikes(false)}
              className="w-full mt-6 bg-red-500 hover:bg-red-600 text-white py-3 rounded-lg font-bold transition"
            >
