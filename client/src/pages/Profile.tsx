...
export default function Profile() {
  ...
  const earningsBySource = useMemo(() => {
    ...
    (earningsQuery.data as Earning[]).forEach((earning) => {
      if (!grouped[earning.source]) {
        grouped[earning.source] = { total: 0, count: 0 };
      }
      grouped[earning.source].total += parseFloat(earning.amount);
      grouped[earning.source].count += 1;
    });
  ...
  const totalEarnings = useMemo(() => {
    return Object.values(earningsBySource).reduce((sum, item) => sum + item.total, 0);
  }, [earningsBySource]);
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end">
          <div className="w-full bg-gray-900 rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-black/50 rounded-lg p-4 mb-6">
              ...
              <p className="text-4xl font-bold text-white mb-2">${totalEarnings.toFixed(2)}</p>
