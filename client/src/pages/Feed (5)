  ...
  useEffect(() => {
    ...
    feedQuery.data.forEach((v: Video) => {
      setVideoCounters((prev) => ({
        ...prev,
        [v.id]: {
          likes: prev[v.id]?.likes ?? v.likes ?? 0,
          comments: prev[v.id]?.comments ?? v.comments ?? 0,
          shares: prev[v.id]?.shares ?? v.shares ?? 0,
          favorites: prev[v.id]?.favorites ?? v.favorites ?? 0,
        },
      }));
    });
  }, [feedQuery.data]);
  ...
  return (
    <div className="bg-black text-white h-screen overflow-hidden flex flex-col">
              ...
              <div className="absolute right-3 top-32 flex flex-col gap-4 z-50">
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-white">
                    {videoCounters[video.id]?.likes || 0}
                  </p>
                </button>
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-white">
                    {videoCounters[video.id]?.comments || 0}
                  </p>
                </button>
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-white">
                    {videoCounters[video.id]?.favorites || 0}
                  </p>
                </button>
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-white">
                    {videoCounters[video.id]?.shares || 0}
                  </p>
                </button>
              </div>
      ...
      <nav className="absolute bottom-0 left-0 right-0 z-40 bg-black/95 border-t border-gray-800 px-2 py-2 flex items-center justify-around">
        ...
        {/* PROFILE */}
        <button
          onClick={() => user?.id ? navigate(`/profile/${user.id}`) : navigate("/login")}
          className="flex flex-col items-center gap-1 p-2 hover:bg-gray-900 rounded-lg transition text-white"
        >
          <User size={24} />
