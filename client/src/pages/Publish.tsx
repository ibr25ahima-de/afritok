...
export default function Publish() {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Si une musique est sélectionnée, on effectue le mixage
      if (selectedMusic) {
        setUploadProgress(20);
        console.log("🎵 Mixage de l'audio avec la musique:", selectedMusic.name);
        
        try {
          // On crée une URL pour le fichier vidéo original
          const videoUrl = URL.createObjectURL(file);
          
          // On appelle mixAudio qui retourne une URL de blob
          const mixedVideoUrl = await mixAudio(videoUrl, selectedMusic.url);
          
          // On récupère le blob à partir de l'URL
          const response = await fetch(mixedVideoUrl);
          const mixedBlob = await response.blob();
          
          // On crée un nouveau fichier à partir du blob mixé
          finalFile = new File([mixedBlob], file.name, { type: 'video/webm' });
          
          // Nettoyage de l'URL temporaire
          URL.revokeObjectURL(videoUrl);
          console.log("✅ Mixage terminé avec succès");
        } catch (mixError) {
          console.error("❌ Erreur lors du mixage audio, utilisation du fichier original:", mixError);
          // En cas d'erreur de mixage, on continue avec le fichier original
        }
      }
