...
export default function Upload() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // ✅ 1. Upload le fichier via FormData (endpoint Express)
      const formData = new FormData();
      formData.append("file", file);
      formData.append("userId", String(user.id));

      console.log("[Upload] Uploading file via FormData...");
      ...
      console.log("[Upload] File uploaded successfully, URL:", videoUrl);

      // ✅ 2. Crée la vidéo via tRPC
      console.log("[Upload] Creating video record via tRPC...");
      ...
      console.log("[Upload] Video record created successfully");

      // ✅ 3. Affiche le message de succès
      ...
      {!file ? (
        <div className="w-full max-w-md space-y-4">
          ...
          <button
            ...
            className="w-full bg-purple-500 py-4 rounded-xl font-bold"
          >
            ✍️ Texte
          </button>
