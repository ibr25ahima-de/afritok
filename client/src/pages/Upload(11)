...
export default function Upload() {
  ...
  const getCameraFilter = () => {
    let filter = `brightness(${100 + beautyLevel * 0.2}%) contrast(${100 + beautyLevel * 0.1}%) saturate(${100 + beautyLevel * 0.15}%)`;
    
    // Add specific filters
    if (activeFilter.id === "bw") filter += " grayscale(100%)";
    if (activeFilter.id === "sepia") filter += " sepia(100%)";
    ...
    // Add effects
    if (activeEffect.id === "glow") filter += " blur(2px) brightness(1.2)";
    if (activeEffect.id === "blur") filter += " blur(5px)";
  ...
  const getCameraTransform = () => {
    let transform = "";
    if (activeEffect.id === "zoom") transform += " scale(1.2)";
    if (activeEffect.id === "mirror") transform += " scaleX(-1)";
    if (activeEffect.id === "shake") transform += " translate(2px, 2px)";
    return transform;
  };
        ...
        <Drawer title="Beauté" onClose={() => setActiveDrawer("none")}>
          <div className="space-y-6 py-4">
            ...
            <div className="grid grid-cols-4 gap-4 opacity-50">
              {["Lissage", "Éclaircir", "Netteté", "Affiner"].map(label => (
                <div key={label} className="flex flex-col items-center gap-2">
                  <div className="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center"><Wand2 size={20}/></div>
            ...
            {FILTERS.map(f => (
              <button key={f.id} onClick={() => setActiveFilter(f)} className={`flex flex-col items-center min-w-[70px] transition-all ${activeFilter.id === f.id ? 'scale-110' : 'opacity-50'}`}>
                <div className={`w-14 h-14 rounded-full border-2 mb-2 ${activeFilter.id === f.id ? 'border-white' : 'border-transparent'} overflow-hidden bg-gray-800`}>
                   {/* Aperçu réel simulé avec une vidéo ou gradient */}
                  <div className="w-full h-full bg-gradient-to-tr from-purple-600 to-blue-400 opacity-30" />
                  <video autoPlay playsInline muted className="absolute inset-0 w-full h-full object-cover" style={{ filter: `brightness(1.1) saturate(1.2)` }} />
                </div>
      ...
      {activeDrawer === "effects" && (
        <Drawer title="Effets" onClose={() => setActiveDrawer("none")}>
          <div className="grid grid-cols-4 gap-4">
            {EFFECTS.map(e => (
              <button key={e.id} onClick={() => { setActiveEffect(e); setActiveDrawer("none"); }} className={`flex flex-col items-center gap-2 ${activeEffect.id === e.id ? 'scale-110' : 'opacity-60'}`}>
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl ${activeEffect.id === e.id ? 'bg-red-500/20 border-2 border-red-500' : 'bg-white/5'}`}>{e.emoji}</div>
