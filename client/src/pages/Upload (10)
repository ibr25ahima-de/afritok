...
import { 
  ...
  Wand2, Search, Play, Pause, Trash2
} from "lucide-react";
...
const FILTERS = [
  ...
  { id: "beauty", name: "Beauté", class: "brightness-110 contrast-105 saturate-110 blur-[0.3px]" },
  { id: "portrait", name: "Portrait", class: "contrast-115 saturate-120" },
  { id: "vintage", name: "Vintage", class: "sepia(0.4) brightness-90" },
  { id: "b&w", name: "N&B", class: "grayscale" }
];

const EFFECTS = [
  { id: "fire", emoji: "🔥", name: "Feu" },
  { id: "heart", emoji: "❤️", name: "Amour" },
  { id: "star", emoji: "⭐", name: "Star" },
  { id: "cool", emoji: "😎", name: "Cool" }
];

const MUSICS = [
  { id: 1, name: "Afrobeat Mix", artist: "Burna", duration: 15 },
  { id: 2, name: "TikTok Viral", artist: "Unknown", duration: 30 },
  { id: 3, name: "Chill Night", artist: "Lofi", duration: 60 }
];
...
export default function Upload() {
  ...
  const [overlayTexts, setOverlayTexts] = useState<{id: number, text: string, x: number, y: number}[]>([]);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      await uploadMutation.mutateAsync({ title: description.slice(0, 20), videoUrl: data.videoUrl });
  ...
  if (step === "capture") return (
    <div className="h-screen bg-black text-white relative overflow-hidden flex flex-col">
      ...
      {/* Sidebar Tools */}
      <div className="absolute right-4 top-20 flex flex-col gap-5 z-10">
        <button onClick={() => setFacingMode(f => f === "user" ? "environment" : "user")} className="flex flex-col items-center gap-1">
          <div className="p-2.5 bg-black/30 rounded-full backdrop-blur-md border border-white/5"><RotateCw size={26}/></div>
          <span className="text-[10px] font-bold uppercase tracking-tighter">Retourner</span>
        </button>
        <button onClick={() => setActiveDrawer("beauty")} className="flex flex-col items-center gap-1">
          <div className="p-2.5 bg-black/30 rounded-full backdrop-blur-md border border-white/5"><Wand2 size={26}/></div>
          <span className="text-[10px] font-bold uppercase tracking-tighter">Beauté</span>
        </button>
        <button onClick={() => setActiveDrawer("filters")} className="flex flex-col items-center gap-1">
          <div className="p-2.5 bg-black/30 rounded-full backdrop-blur-md border border-white/5 text-yellow-400"><Sparkles size={26}/></div>
          <span className="text-[10px] font-bold uppercase tracking-tighter">Filtres</span>
        </button>
        <button onClick={() => setActiveDrawer("effects")} className="flex flex-col items-center gap-1">
          <div className="p-2.5 bg-black/30 rounded-full backdrop-blur-md border border-white/5"><Smile size={26}/></div>
          <span className="text-[10px] font-bold uppercase tracking-tighter">Effets</span>
        </button>
      </div>
      ...
      {activeDrawer === "music" && (
        <Drawer title="Musique" onClose={() => setActiveDrawer("none")}>
          <div className="space-y-3 max-h-[40vh] overflow-y-auto">
            {MUSICS.map(m => (
              <div key={m.id} onClick={() => { setSelectedMusic(m); setActiveDrawer("none"); }} className="p-4 bg-white/5 rounded-2xl flex items-center justify-between active:scale-95 transition-transform">
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
            {FILTERS.map(f => (
              <button key={f.id} onClick={() => setActiveFilter(f)} className={`flex flex-col items-center min-w-[70px] transition-all ${activeFilter.id === f.id ? 'scale-110' : 'opacity-50'}`}>
                <div className={`w-14 h-14 rounded-full border-2 mb-2 ${activeFilter.id === f.id ? 'border-white' : 'border-transparent'} overflow-hidden bg-gray-800`}>
                  <div className={`w-full h-full ${f.class} bg-gradient-to-tr from-purple-600 to-blue-400`}/>
                </div>
      ...
      {activeDrawer === "effects" && (
        <Drawer title="Effets" onClose={() => setActiveDrawer("none")}>
          <div className="grid grid-cols-4 gap-4">
            {EFFECTS.map(e => (
              <button key={e.id} onClick={() => { setActiveEffect(e); setActiveDrawer("none"); }} className="flex flex-col items-center gap-2">
                <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center text-3xl">{e.emoji}</div>
      ...
      <div className="mt-auto pb-10 pt-4 z-10 bg-gradient-to-t from-black/80 to-transparent">
        ...
        <div className="flex items-center justify-evenly px-6">
          <button className="flex flex-col items-center gap-1 opacity-80">
             <div className="w-12 h-12 rounded-full border-2 border-white/30 bg-black/40 flex items-center justify-center text-2xl">{activeEffect?.emoji || "✨"}</div>
          ...
          <div className="relative flex items-center justify-center">
            <div className={`w-20 h-20 rounded-full border-4 ${isRecording ? 'border-red-500 animate-pulse scale-125' : 'border-white'} flex items-center justify-center p-1.5 transition-all duration-300`}>
              <button onClick={handleCaptureBtn} className={`w-full h-full bg-red-500 rounded-full shadow-2xl transition-all ${isRecording ? 'scale-75 rounded-lg' : 'scale-100'}`} />
            </div>
          </div>
          ...
          <button onClick={() => inputRef.current?.click()} className="flex flex-col items-center gap-1 opacity-80">
            ...
            <div className="w-12 h-12 rounded-xl border-2 border-white/30 bg-black/40 flex items-center justify-center overflow-hidden"><ImageIcon size={24}/></div>
            <span className="text-[10px] font-bold uppercase">Déposer</span>
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="h-screen bg-black text-white relative flex flex-col overflow-hidden">
      ...
      {isAddingText && (
        <div className="absolute inset-0 z-[60] bg-black/80 flex flex-col p-6 animate-in fade-in duration-200">
          <div className="flex justify-end"><button onClick={() => { if(tempText) setOverlayTexts([...overlayTexts, {id: Date.now(), text: tempText, x: 50, y: 50}]); setIsAddingText(false); setTempText(""); }} className="bg-white text-black px-4 py-1 rounded-full font-bold">Terminer</button></div>
