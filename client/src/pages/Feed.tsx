...
/** ================= STATE ================= */ const [videos, setVideos] = useState<Video[]>([]); const [currentVideoIndex, setCurrentVideoIndex] = useState(0); const [offset, setOffset] = useState(0); const [showComments, setShowComments] = useState(false); const [showShare, setShowShare] = useState(false); const [touchStart, setTouchStart] = useState(0); const [heartVideoId, setHeartVideoId] = useState<number | null>(null); const [tab, setTab] = useState<"forYou" | "following">("forYou");

const videoRefs = useRef<Record<number, HTMLVideoElement | null>>({}); const containerRef = useRef<HTMLDivElement>(null); const observerRef = useRef<IntersectionObserver | null>(null); const lastTapRef = useRef<number>(0);
...
/** ================= LOAD FEED ================= */ useEffect(() => { const incoming = feedQuery.data; if (!incoming) return;
...
setVideos((prev) => prev.map((v) => (v.id === video.id ? { ...v, likes: (v.likes || 0) + (wasLiked ? -1 : 1) } : v)));
...
/** ================= DOUBLE TAP ================= */ const handleDoubleTap = (video: Video) => { const now = Date.now(); if (now - lastTapRef.current < 300) { handleLike(video); setHeartVideoId(video.id); setTimeout(() => setHeartVideoId(null), 900); } lastTapRef.current = now; };

/** ================= SWIPE ================= */ const handleSwipe = useCallback( (start: number, end: number) => { const distance = start - end; if (distance > 50 && currentVideoIndex < videos.length - 1) setCurrentVideoIndex((p) => p + 1); if (distance < -50 && currentVideoIndex > 0) setCurrentVideoIndex((p) => p - 1); }, [currentVideoIndex, videos.length] );

const currentVideo = videos[currentVideoIndex]; if (!isAuthenticated) return null;

return ( <div className="min-h-screen bg-black text-white"> {/* HEADER */} <header className="fixed top-0 left-0 right-0 z-50 bg-black/70 backdrop-blur"> <div className="flex justify-between items-center px-4 py-3"> <Search onClick={() => navigate("/search")} />

<div className="flex gap-6 text-white font-semibold">
        <button onClick={() => setTab("following")} className={tab === "following" ? "opacity-100" : "opacity-50"}>
          Following
        </button>
        <button onClick={() => setTab("forYou")} className={tab === "forYou" ? "opacity-100" : "opacity-50"}>
          For You
        </button>
      </div>
        ...
        <div key={video.id} className="h-screen" style={{ scrollSnapAlign: "start" }}>
          <div className="relative w-full h-full max-w-md mx-auto">
            ...
            <div className="absolute right-3 bottom-24 flex flex-col items-center gap-5">
              ...
              <button onClick={() => { setCurrentVideoIndex(i); setShowComments(true); }}>
                <MessageCircle size={28} />
                <p className="text-xs">{video.comments || 0}</p>
              </button>

              <button onClick={() => { setCurrentVideoIndex(i); setShowShare(true); }}>
                <Share2 size={28} />
  ...
  <nav className="fixed bottom-0 left-0 right-0 bg-black border-t border-gray-800 flex justify-around py-3">
    ...
    <button onClick={() => navigate("/upload")}><PlusSquare size={32} /></button>
