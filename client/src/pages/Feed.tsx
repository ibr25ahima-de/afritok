...
import { Heart, MessageCircle, Share2, User, Loader2, Bell } from "lucide-react";
...
export default function Feed() {
  ...
  useEffect(() => {
    ...
    setVideos((prev) => {
      const newOnes = feedQuery.data.filter((v) => !prev.some((p) => p.id === v.id));
      return [...prev, ...newOnes];
    });
  }, [feedQuery.data]);
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-gray-800 bg-black/80 backdrop-blur-md">
        <div className="flex justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            {APP_LOGO && <img src={APP_LOGO} className="h-8 w-8 rounded" />}
            <span className="text-white font-bold">{APP_TITLE}</span>
          </div>
          <Button variant="ghost" onClick={() => navigate(`/profile/${user?.id}`)}>
