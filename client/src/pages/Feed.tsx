...
export default function Feed() {
  ...
  /** ================= API ================= */
  // ❌ SUPPRIMÉ : const feedQuery = trpc.video.feed.useQuery({ limit: 20, offset });
  ...
  /** ================= LOAD VIDEOS (NOUVEAU SYSTÈME) ================= */
  useEffect(() => {
    if (!trpcVideos || trpcVideos.length === 0) return;

    setVideos(trpcVideos);

    const counters: Record<number, any> = {};
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Note: feedQuery est supprimé, on pourrait adapter ici avec trpcVideos si besoin de pagination
  ...
  if (trpcLoading) return <div className="min-h-screen bg-black text-white flex items-center justify-center">Loading...</div>;
  ...
  return (
    <div className="bg-black text-white h-screen overflow-hidden flex flex-col">
      ...
      {/* NAVIGATION BAR */}
      <nav className="bg-black border-t border-white/10 px-6 py-3 flex items-center justify-between z-40">
        <button onClick={() => navigate("/feed")} className="flex flex-col items-center gap-1 text-white">
          <Home size={24} />
          <span className="text-[10px]">Accueil</span>
        </button>
        <button onClick={() => navigate("/discovery")} className="flex flex-col items-center gap-1 text-gray-500">
          <Users size={24} />
        ...
        <button onClick={() => navigate("/profile")} className="flex flex-col items-center gap-1 text-gray-500">
          <User size={24} />
