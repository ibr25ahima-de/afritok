...
export default function Feed() {
  ...
  return (
    <div className="bg-black text-white h-screen overflow-hidden flex flex-col relative">
      ...
      {showDeleteDialog && selectedVideoId && <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/75 px-5" role="dialog" aria-modal="true" aria-labelledby="delete-video-title"><div className="relative z-10 w-full max-w-sm rounded-2xl bg-gray-900 border border-gray-700 p-5 shadow-2xl"><h2 id="delete-video-title" className="text-lg font-bold">Supprimer la vidéo ?</h2><p className="mt-2 text-sm text-gray-400">Cette action est définitive.</p><button type="button" onClick={handleDeleteVideo} disabled={deleteVideoMutation.isPending} className="mt-5 w-full rounded-xl bg-red-600 px-4 py-3 font-semibold disabled:opacity-50">{deleteVideoMutation.isPending ? "Suppression…" : "Supprimer la vidéo"}</button><button type="button" onClick={() => { setShowDeleteDialog(false); setSelectedVideoId(null); }} disabled={deleteVideoMutation.isPending} className="mt-3 w-full rounded-xl bg-gray-800 px-4 py-3 font-semibold disabled:opacity-50">Annuler</button></div></div>}
    </div>
  );
}
