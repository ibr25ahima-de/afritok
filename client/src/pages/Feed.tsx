...
export default function Feed() {
  ...
  return (
    <div className="bg-black text-white h-screen overflow-hidden flex flex-col">
      ...
      >
        {videos.length === 0 ? (
          ...
          videos.map((video, i) => (
            ...
            >
              {/* VIDEO - FIX: Check if videoUrl exists */}
              {video.videoUrl ? (
                <video
      ...
      {/* BOTTOM NAVIGATION WITH UPLOAD BUTTON */}
      <nav className="absolute bottom-0 left-0 right-0 z-40 bg-black/90 border-t border-gray-800 px-4 py-3 flex items-center justify-around">
        {/* HOME */}
        <button
          onClick={() => navigate("/feed")}
          className="p-2 hover:bg-gray-900 rounded-full transition text-white"
        >
          <Home size={24} />
        </button>
