...
export default function InstantWithdraw() {
  ...
  const handleWithdraw = async () => {
    try {
      ...
      // ✅ Correction : conversion number + country "CI"
      const result = await withdrawMutation.mutateAsync({
        amount: Number(withdrawAmount), // ✅ convertir en number
        phoneNumber,
        provider: selectedProvider,
        country: "CI", // ✅ obligatoire (Côte d'Ivoire)
      });

      // ✅ Correction : Mise à jour du résultat et passage à l'étape 3
      setWithdrawalResult(result);
      setStep(3);

    } catch (error: any) {
      // ✅ Correction : Affichage de la vraie erreur
