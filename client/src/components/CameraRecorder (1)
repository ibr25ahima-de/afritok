...
export const CameraRecorder: React.FC<CameraRecorderProps> = ({
  ...
  const handleCapture = () => {
    if (selectedDuration === "Photo") {
      ...
      mr.start(1000); // Collecter les données chaque seconde
  ...
  return (
    <div className="h-screen bg-black text-white relative overflow-hidden flex flex-col">
      ...
      <div className="relative p-4 flex justify-between items-start z-20">
        ...
        <button 
          onClick={onOpenMusic} 
          className="bg-black/30 backdrop-blur-lg px-4 py-2 rounded-full flex items-center gap-2 text-sm font-bold border border-white/10"
        >
          <Music size={16}/> 
          <span className="max-w-[120px] truncate">{selectedMusic?.name || "Ajouter un son"}</span>
        </button>

        {/* Barre latérale droite (Les 7 boutons) */}
        <div className="flex flex-col gap-4 items-center">
           <button onClick={() => setFacingMode(f => f === "user" ? "environment" : "user")} className="flex flex-col items-center gap-1">
             <div className="p-2.5 bg-black/20 rounded-full"><RotateCw size={26}/></div>
             <span className="text-[10px] font-bold shadow-sm">Retourner</span>
           </button>
           
           <button className="flex flex-col items-center gap-1">
             <div className="p-2.5 bg-black/20 rounded-full"><Gauge size={26}/></div>
             <span className="text-[10px] font-bold shadow-sm">Vitesse</span>
           </button>
           
           <button className="flex flex-col items-center gap-1">
             <div className="p-2.5 bg-black/20 rounded-full"><Wand2 size={26}/></div>
             <span className="text-[10px] font-bold shadow-sm">Filtres</span>
           </button>
           
           <button className="flex flex-col items-center gap-1">
             <div className="p-2.5 bg-black/20 rounded-full"><Timer size={26}/></div>
             <span className="text-[10px] font-bold shadow-sm">Retardateur</span>
           </button>

           <button onClick={() => setFlashEnabled(!flashEnabled)} className="flex flex-col items-center gap-1">
             <div className="p-2.5 bg-black/20 rounded-full">
               {flashEnabled ? <Zap size={26} className="text-yellow-400"/> : <ZapOff size={26}/>}
             </div>
             <span className="text-[10px] font-bold shadow-sm">Flash</span>
           </button>

           <button className="flex flex-col items-center gap-1">
             <div className="p-2.5 bg-black/20 rounded-full"><Layout size={26}/></div>
             <span className="text-[10px] font-bold shadow-sm">Montage</span>
           </button>

           <button onClick={() => setShowMoreIcons(!showMoreIcons)} className="flex flex-col items-center gap-1">
             <div className="p-2.5 bg-black/20 rounded-full">
               {showMoreIcons ? <ChevronUp size={26}/> : <ChevronDown size={26}/>}
             </div>
           </button>
        </div>
      </div>
      ...
      {/* Zone Basse */}
      <div className="mt-auto pb-10 z-20 bg-gradient-to-t from-black/60 to-transparent">
        {/* Sélecteur de durée */}
        <div className="flex justify-center gap-6 mb-8 overflow-x-auto no-scrollbar px-6">
          {["10 min", "60 s", "15 s", "Photo"].map(m => (
            <button 
              key={m} 
              onClick={() => setSelectedDuration(m)} 
              className={`text-[13px] font-black whitespace-nowrap px-4 py-1.5 rounded-full transition-all ${selectedDuration === m ? "bg-white text-black scale-110" : "text-white/60 bg-black/20"}`}
            >
              {m}
            </button>
          ))}
        ...
        <div className="flex items-center justify-around px-4">
          ...
          <div className={`w-22 h-22 rounded-full border-[6px] ${isRecording ? 'border-red-500/30 scale-110' : 'border-white'} flex items-center justify-center p-2 transition-all duration-300`}>
            <button 
              ...
              className={`w-full h-full bg-red-500 rounded-full shadow-lg active:scale-90 transition-all ${isRecording ? 'scale-60 rounded-xl' : ''}`} 
            />
          </div>

          <button className="flex flex-col items-center gap-1 group">
            <div className="w-14 h-14 rounded-xl border-2 border-white/40 overflow-hidden bg-black/40 flex items-center justify-center group-active:scale-90 transition-transform">
               <div className="w-10 h-10 bg-gradient-to-tr from-blue-500 via-purple-500 to-pink-500 rounded-md"/>
            </div>
