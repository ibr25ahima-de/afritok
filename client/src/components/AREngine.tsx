...
const DEFAULT_BEAUTY = { smoothSkin: 0.82, brightenSkin: 0.08, enlargeEyes: 0, slimFace: 0, whitenTeeth: 0, enlargeLips: 0, symmetry: 0 };
...
function applySkinPolish(ctx: CanvasRenderingContext2D, landmarks: NormalizedLandmark[], width: number, height: number, buffer: HTMLCanvasElement) {
  ...
  const pad = Math.max(8, Math.round(Math.min(face.width,face.height)*.05));
  ...
  bc.clearRect(0,0,w,h); bc.filter="blur(5px)"; bc.drawImage(ctx.canvas,l,t,w,h,0,0,w,h); bc.filter="none";
  ctx.save(); polygonFace(ctx,points); ctx.clip(); ctx.globalAlpha=.55; ctx.drawImage(buffer,l,t); ctx.restore();
}
...
export const AREngine:React.FC<AREngineProps>=({videoRef,activeEffect})=>{
 ...
 const detect=useCallback((video:HTMLVideoElement,t:number)=>{
  ...
  try{const r=lm.detectForVideo(video,t); const cur=r.faceLandmarks?.[0]; if(cur)previousRef.current=smoothLandmarks(cur,previousRef.current,.45);}
  catch(e){console.error("[AREngine] detect error",e)} return previousRef.current;
 },[]);
 useEffect(()=>{let cancelled=false;(async()=>{if(initRef.current)return;initRef.current=true;try{const vision=await FilesetResolver.forVisionTasks(WASM_URL);const lm=await FaceLandmarker.createFromOptions(vision,{baseOptions:{modelAssetPath:MODEL_URL},runningMode:"VIDEO",numFaces:1,minFaceDetectionConfidence:.25,minFacePresenceConfidence:.25,minTrackingConfidence:.25});if(cancelled){lm.close();return}landmarkerRef.current=lm;console.log("[AREngine] ready")}catch(e){console.error("[AREngine] init failed",e)}finally{initRef.current=false}})();return()=>{cancelled=true;if(animationRef.current)cancelAnimationFrame(animationRef.current);landmarkerRef.current?.close();landmarkerRef.current=null;previousRef.current=null}},[]);
