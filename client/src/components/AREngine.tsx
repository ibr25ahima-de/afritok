  ...
  const renderFrame = useCallback(() => {
    ...
    if (activeEffect?.beautyConfig) {
      ...
      if (cfg.smoothSkin && cfg.smoothSkin > 0) {
        if (faceB) {
          ...
          ctx.save();
          // Blur la zone du visage pour lisser la peau
          ctx.filter = `blur(${cfg.smoothSkin * 4}px)`;
          ctx.globalAlpha = cfg.smoothSkin * 0.35;
          ...
          if (cfg.brightenSkin && cfg.brightenSkin > 0) {
            ctx.save();
            ctx.globalAlpha = cfg.brightenSkin * 0.12;
            const gradient = ctx.createRadialGradient(
              faceB.cx * cw, faceB.cy * ch, 0,
              faceB.cx * cw, faceB.cy * ch, faceB.w * cw * 0.5
            );
            gradient.addColorStop(0, 'rgba(255, 235, 220, 0.5)');
      ...
      if (cfg.enlargeEyes && cfg.enlargeEyes > 0) {
        ...
        const factor = 1 + cfg.enlargeEyes * 0.4;
        const glowRadius = Math.max(leftW, rightW) * factor * 2.5;

        ctx.save();
        
        // Glow blanc autour des yeux (effet d'agrandissement visuel)
        [leftC, rightC].forEach((c, idx) => {
          ...
          glow.addColorStop(0, `rgba(255, 255, 255, ${cfg.enlargeEyes * 0.2})`);
          glow.addColorStop(0.3, `rgba(255, 255, 255, ${cfg.enlargeEyes * 0.1})`);
        ...
        // Redessiner la zone des yeux avec un léger agrandissement
        [leftC, rightC].forEach((c, idx) => {
          const r = idx === 0 ? leftW : rightW;
          const scaleX = factor;
          const scaleY = factor * 0.85;
          
          ctx.save();
          ctx.globalAlpha = cfg.enlargeEyes * 0.15;
      ...
      if (cfg.slimFace && cfg.slimFace > 0) {
        if (faceB) {
          ctx.save();
          const edgeFade = cfg.slimFace * 0.45;
          
          // Assombrir les bords gauche du visage
          const leftGrad = ctx.createLinearGradient(
            faceB.left * cw, 0,
            (faceB.left + faceB.w * 0.25) * cw, 0
          );
          leftGrad.addColorStop(0, `rgba(0, 0, 0, ${edgeFade})`);
          leftGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
          ctx.fillStyle = leftGrad;
          ctx.fillRect(faceB.left * cw, faceB.top * ch, faceB.w * 0.25 * cw, faceB.h * ch);

          // Assombrir les bords droit du visage
          const rightGrad = ctx.createLinearGradient(
            (faceB.right - faceB.w * 0.25) * cw, 0,
            faceB.right * cw, 0
          );
          rightGrad.addColorStop(0, 'rgba(0, 0, 0, 0)');
          rightGrad.addColorStop(1, `rgba(0, 0, 0, ${edgeFade})`);
          ctx.fillStyle = rightGrad;
          ctx.fillRect((faceB.right - faceB.w * 0.25) * cw, faceB.top * ch, faceB.w * 0.25 * cw, faceB.h * ch);

          // Highlight central (rend le milieu du visage plus lumineux = plus fin)
          const centerGlow = ctx.createRadialGradient(
            faceB.cx * cw, faceB.cy * ch, 0,
            faceB.cx * cw, faceB.cy * ch, faceB.w * cw * 0.35
          );
          centerGlow.addColorStop(0, `rgba(255, 240, 225, ${cfg.slimFace * 0.08})`);
          centerGlow.addColorStop(1, 'rgba(255, 240, 225, 0)');
      ...
      if (cfg.whitenTeeth && cfg.whitenTeeth > 0) {
        ...
        ctx.save();
        ctx.globalAlpha = cfg.whitenTeeth * 0.4;
        const whitenGlow = ctx.createRadialGradient(mx, my, 0, mx, my, mouthW * 0.5);
        whitenGlow.addColorStop(0, 'rgba(255, 255, 255, 0.6)');
        whitenGlow.addColorStop(1, 'rgba(255, 255, 255, 0)');
        ctx.fillStyle = whitenGlow;
        ctx.fillRect(mx - mouthW, my - mouthW * 0.4, mouthW * 2, mouthW * 0.8);
        ctx.restore();
      }
      ...
      if (cfg.enlargeLips && cfg.enlargeLips > 0) {
        ...
        ctx.save();
        // Halo rose autour des lèvres
        const lipHalo = ctx.createRadialGradient(lx, ly, 0, lx, ly, lipW * 0.7);
        lipHalo.addColorStop(0, `rgba(255, 130, 130, ${cfg.enlargeLips * 0.25})`);
      ...
      if (cfg.symmetry && cfg.symmetry > 0) {
        if (faceB) {
          ...
          ctx.globalAlpha = cfg.symmetry * 0.08;
    ...
    if (activeEffect?.visualConfig) {
      ...
      if (vCfg.filter && vCfg.intensity) {
        ctx.save();
        ctx.globalAlpha = vCfg.intensity * 0.2;
        
        // Interpréter le filtre et appliquer un overlay coloré
        if (vCfg.filter.includes('hue-rotate(180')) {
          ...
          ctx.fillStyle = 'rgba(30, 0, 60, 0.12)';
      ...
      if (vCfg.overlay && faceB && landmarks.length > 0) {
        ...
        switch (vCfg.overlay) {
          ...
          case 'bear-ears': {
            ...
            const earW = dist(bLC, bRC) * cw * 0.3;
            const earH = animalType === 'rabbit' ? earW * 3.5 : earW * 1.8;
            const earOffset = animalType === 'rabbit' ? earH * 0.6 : earH * 0.35;
            ...
            // Oreille gauche
            ctx.beginPath();
            ctx.ellipse(bLC.x * cw - earW * 0.2, bLC.y * ch - earOffset, earW * 0.7, earH, -0.25, 0, 2 * Math.PI);
            ctx.fillStyle = c.outer;
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(bLC.x * cw - earW * 0.2, bLC.y * ch - earOffset, earW * 0.35, earH * 0.55, -0.25, 0, 2 * Math.PI);
            ctx.fillStyle = c.inner;
            ctx.fill();
            
            // Oreille droite
            ctx.beginPath();
            ctx.ellipse(bRC.x * cw + earW * 0.2, bRC.y * ch - earOffset, earW * 0.7, earH, 0.25, 0, 2 * Math.PI);
            ctx.fillStyle = c.outer;
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(bRC.x * cw + earW * 0.2, bRC.y * ch - earOffset, earW * 0.35, earH * 0.55, 0.25, 0, 2 * Math.PI);
            ctx.fillStyle = c.inner;
            ctx.fill();
            ...
            ctx.beginPath();
            ctx.ellipse(noseC.x * cw, noseC.y * ch + 15 * ch / 720, 6 * cw / 1280, 4 * ch / 720, 0, 0, 2 * Math.PI);
          ...
          case 'hearts': {
            ...
            for (let i = 0; i < 8; i++) {
              ...
              const radius = faceB!.w * cw * (0.4 + 0.1 * Math.sin(timeRef.current * 2 + i));
              const hx = fc.x * cw + Math.cos(angle) * radius;
              const hy = fc.y * ch + Math.sin(angle) * radius;
              const size = 16 + 4 * Math.sin(timeRef.current * 3 + i);
              ctx.font = `${size}px serif`;
              ctx.globalAlpha = 0.5 + 0.3 * Math.sin(timeRef.current * 2 + i);
          ...
          case 'sparkles': {
            ...
            sparklePts.forEach((pt, i) => {
              const x = pt.x * cw;
              const y = pt.y * ch;
              const alpha = 0.4 + 0.6 * Math.abs(Math.sin(timeRef.current * 3 + i * 1.5));
              const size = 4 + 4 * Math.abs(Math.sin(timeRef.current * 2 + i));
          ...
          case 'glitch': {
            if (Math.random() > 0.6) {
              ...
              ctx.restore();
              ctx.fillStyle = Math.random() > 0.5 ? 'rgba(255,0,0,0.08)' : 'rgba(0,255,255,0.08)';
          ...
          case 'fire': {
            ctx.save();
            const jawPts = LANDMARKS.faceOval.slice(15, 25).map(i => landmarks[i]);
            for (let i = 0; i < 25; i++) {
              const pt = jawPts[i % jawPts.length];
              const x = pt.x * cw + (Math.random() - 0.5) * 40;
              const y = pt.y * ch + Math.random() * 25;
          ...
          case 'vampire': {
            ...
            // Croc gauche
            ctx.beginPath();
            ctx.moveTo(cx - 10 * cw / 1280, cy - 2 * ch / 720);
            ctx.lineTo(cx - 6 * cw / 1280, cy + 12 * ch / 720);
            ctx.lineTo(cx - 2 * cw / 1280, cy - 2 * ch / 720);
            ctx.fill();
            // Croc droit
            ctx.beginPath();
            ctx.moveTo(cx + 2 * cw / 1280, cy - 2 * ch / 720);
            ctx.lineTo(cx + 6 * cw / 1280, cy + 12 * ch / 720);
            ctx.lineTo(cx + 10 * cw / 1280, cy - 2 * ch / 720);
            ctx.fill();
            ctx.restore();
            break;
          }
          ...
          case 'ghost': {
            ctx.save();
            ctx.globalAlpha = 0.05 + 0.03 * Math.sin(timeRef.current * 2);
            ctx.fillStyle = '#ffffff';
            ctx.fillRect(0, 0, cw, ch);
            ctx.restore();
            break;
          ...
          case 'anime': {
            ...
            if (faceB) {
              ...
              [eyeL, eyeR].forEach(e => {
                ...
                ctx.fillStyle = grad;
                ctx.beginPath();
                ctx.arc(e.x * cw, e.y * ch, 18 * cw / 1280, 0, 2 * Math.PI);
                ctx.fill();
              });
              ctx.restore();
            }
          ...
          case 'trippy': {
            if (faceB) {
              ...
              for (let ring = 1; ring <= 5; ring++) {
                const radius = faceB.w * cw * 0.15 * ring;
                const hue = (timeRef.current * 100 + ring * 60) % 360;
