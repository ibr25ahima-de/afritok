...
export function VideoTimeline({
  ...
  const safeStart = clamp(trimStart, 0, safeDuration);
  ...
  useEffect(() => {
    const move = (event: PointerEvent) => {
      ...
      const minimumGap = Math.min(0.05, safeDuration / 100);

      if (dragging === "start") {
        const nextStart = clamp(value, 0, safeEnd - minimumGap);
        onTrimChange(nextStart, safeEnd);
        onCurrentTimeChange(nextStart);
      } else if (dragging === "end") {
        const nextEnd = clamp(value, safeStart + minimumGap, safeDuration);
        onTrimChange(safeStart, nextEnd);
  ...
  }, [dragging, safeDuration, safeStart, safeEnd, current]);
  ...
  const handleCutTap = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (blocked) {
      onCutsChange(cutSegments.filter((range) => range !== blocked));
      onCurrentTimeChange(blocked.start);
      return;
    }
    onCurrentTimeChange(clamp(value, safeStart, safeEnd));
  };

  return (
    <section className="w-full select-none bg-black text-white" aria-label="Montage vidéo">
      ...
      <div
        ref={trackRef}
        className="relative mx-3 h-[76px] overflow-hidden rounded-lg bg-white/10 touch-none"
        onPointerDown={handleCutTap}
      >
        ...
        {cutSegments.map((range) => (
          <div
            ...
            className="pointer-events-none absolute inset-y-0 bg-black/75"
            style={{ left: `${(range.start / safeDuration) * 100}%`, width: `${((range.end - range.start) / safeDuration) * 100}%` }}
          />
        ))}

        <div
          className="absolute inset-y-0 z-10 border-x-2 border-white"
        ...
        <button
          ...
          className="absolute top-0 bottom-0 z-30 w-5 -translate-x-1/2 touch-none"
          style={{ left: `${startPercent}%` }}
          onPointerDown={(event) => { event.stopPropagation(); event.currentTarget.setPointerCapture?.(event.pointerId); setDragging("start"); }}
        >
          <span className="mx-auto block h-full w-1 rounded-full bg-white" />
          <span className="absolute left-1/2 top-1/2 h-9 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white shadow" />
        </button>
        ...
        <button
          ...
          onPointerDown={(event) => { event.stopPropagation(); event.currentTarget.setPointerCapture?.(event.pointerId); setDragging("end"); }}
        >
          <span className="mx-auto block h-full w-1 rounded-full bg-white" />
          <span className="absolute left-1/2 top-1/2 h-9 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white shadow" />
        </button>
        ...
        <button
          ...
          className="absolute top-0 bottom-0 z-40 w-4 -translate-x-1/2 touch-none"
          style={{ left: `${playheadPercent}%` }}
          onPointerDown={(event) => { event.stopPropagation(); event.currentTarget.setPointerCapture?.(event.pointerId); setDragging("playhead"); }}
        >
      ...
      <div className="flex items-center justify-between px-3 py-2 text-[10px] text-white/55">
