...
export const CameraRecorder: React.FC<CameraRecorderProps> = ({
  ...
  // --- AR EFFECTS CONTROL ---
  const handleAREffectSelect = (effect: AREffect) => {
    if (!effect) {
      setActiveAREffect(null);
      return;
    }
  ...
  // --- CSS filter combiné (filtre couleur + effet AR visuel) ---
  const getCombinedFilter = (): string => {
    const colorFilter = activeFilter?.cssFilter || 'none';
    if (activeAREffect?.visualConfig?.filter && colorFilter === 'none') {
      return activeAREffect.visualConfig.filter;
    }
    if (activeAREffect?.visualConfig?.filter && colorFilter !== 'none') {
      return `${colorFilter} ${activeAREffect.visualConfig.filter}`;
    }
    return colorFilter;
  };
  ...
  return (
    <div className="h-screen bg-black text-white relative overflow-hidden flex flex-col">
      {/* Flux vidéo avec filtre combiné */}
      <video 
        ref={videoRef} 
        autoPlay 
        playsInline 
      ...
      {/* Canvas de détection faciale (débogage) */}
      <canvas ref={faceCanvasRef} className="absolute inset-0 w-full h-full pointer-events-none z-5 opacity-30" />
      ...
      <div className="relative p-4 flex justify-between items-start z-20">
        ...
        <div className="flex flex-col gap-5 items-center">
           ...
           {/* FILTRES - Panneau filtres couleur */}
      ...
      <div className={`absolute left-0 right-0 z-20 transition-all duration-300 ${showAREffectsPanel ? 'bottom-[200px]' : 'bottom-[60px]'}`}>
      ...
      {/* BARRE NOIRE EN BAS : Avatar + PUBLIER + CRÉER + EFFETS */}
      <div className={`absolute bottom-0 left-0 right-0 z-30 bg-black flex items-center px-4 transition-all duration-300 ${showAREffectsPanel ? 'h-[50px]' : 'h-[50px]'}`}>
        ...
        {/* Bouton EFFETS au centre (comme TikTok) */}
        <button 
      ...
      {/* PANNEAU EFFETS AR (TikTok-style - en bas de l'écran) */}
      {showAREffectsPanel && (
        <div className="absolute bottom-[50px] left-0 right-0 z-40">
          <EffectsPanel
            selectedEffect={activeAREffect}
            onSelectEffect={handleAREffectSelect}
            onClose={() => setShowAREffectsPanel(false)}
          />
        </div>
      )}
      ...
      {/* BIBLIOTHÈQUE D'EFFETS CLASSIQUES */}
      {showEffectsPanel && (
        <div className="absolute bottom-0 left-0 right-0 z-40 bg-black/95 backdrop-blur-lg rounded-t-2xl max-h-[60vh] overflow-y-auto p-4">
          <div className="flex items-center justify-between mb-3">
