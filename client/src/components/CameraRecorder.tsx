...
export const CameraRecorder: React.FC<CameraRecorderProps> = ({
  ...
  const handleCapture = () => {
    if (selectedDuration === "Photo") {
      ...
      if (selectedMusic && musicPlayerRef.current) {
      ...
      <div className="relative p-4 flex justify-between items-start z-20">
        ...
        <div className="flex flex-col gap-5 items-center">
           <button onClick={() => setFacingMode(f => f === "user" ? "environment" : "user")} className="flex flex-col items-center gap-1">
             <div className="p-2.5 bg-black/20 rounded-full"><RefreshCw size={26}/></div>
           ...
           <button onClick={() => setActiveSpeed(s => s === "1x" ? "2x" : "1x")} className="flex flex-col items-center gap-1">
             <div className={`p-2.5 rounded-full ${activeSpeed !== "1x" ? "bg-yellow-400 text-black" : "bg-black/20"}`}><Gauge size={26}/></div>
             <span className="text-[10px] font-bold">Vitesse</span>
           </button>
           <button className="flex flex-col items-center gap-1"><div className="p-2.5 bg-black/20 rounded-full"><Wand2 size={26}/></div><span className="text-[10px] font-bold">Filtres</span></button>
           <button onClick={() => setTimerValue(v => v === 0 ? 3 : 0)} className="flex flex-col items-center gap-1">
             <div className={`p-2.5 rounded-full ${timerValue > 0 ? "bg-yellow-400 text-black" : "bg-black/20"}`}><Timer size={26}/></div>
           ...
           <button onClick={() => setFlashEnabled(!flashEnabled)} className="flex flex-col items-center gap-1">
             <div className={`p-2.5 rounded-full ${flashEnabled ? "bg-yellow-400 text-black" : "bg-black/20"}`}>{flashEnabled ? <Zap size={26}/> : <ZapOff size={26}/>}</div>
             <span className="text-[10px] font-bold">Flash</span>
           </button>
           <button className="flex flex-col items-center gap-1"><div className="p-2.5 bg-black/20 rounded-full"><Layout size={26}/></div><span className="text-[10px] font-bold">Montage</span></button>
      ...
      <div className="mt-auto pb-10 z-20 bg-gradient-to-t from-black/80 to-transparent">
        <div className="flex justify-center gap-8 mb-8 overflow-x-auto no-scrollbar px-6">
          {["10 min", "60 s", "15 s", "Photo"].map(m => (
            <button key={m} onClick={() => setSelectedDuration(m)} className={`text-[14px] font-black whitespace-nowrap ${selectedDuration === m ? "text-white scale-110" : "text-white/40"}`}>{m}</button>
          ))}
        </div>
        <div className="flex items-center justify-around px-4">
          <button onClick={onOpenEffects} className="flex flex-col items-center gap-1">
            <div className="w-14 h-14 rounded-xl border-2 border-white/40 overflow-hidden bg-black/40 flex items-center justify-center"><Sparkles size={30} className="text-yellow-400"/></div>
            <span className="text-[11px] font-bold">Effets</span>
          </button>
          <div className={`w-22 h-22 rounded-full border-[6px] ${isRecording ? 'border-red-500/30 scale-110' : 'border-white'} flex items-center justify-center p-2 transition-all`}>
          ...
          <button className="flex flex-col items-center gap-1">
            <div className="w-14 h-14 rounded-xl border-2 border-white/40 overflow-hidden bg-black/40 flex items-center justify-center">
               <div className="w-10 h-10 bg-gradient-to-tr from-blue-500 via-purple-500 to-pink-500 rounded-md flex items-center justify-center"><ImageIcon size={20} className="text-white/50"/></div>
            </div>
