...
export const CameraRecorder: React.FC<CameraRecorderProps> = ({
  ...
  const toggleFlash = async () => {
    ...
    // Accès au flux vidéo pour activer la torche
  ...
  useEffect(() => {
    ...
    // Préparer le lecteur de musique
  ...
  const handleCapture = async () => {
    if (selectedDuration === "Photo") {
      ...
      // Lancer la musique en même temps que l'enregistrement
      if (musicElement) {
    ...
    <div className="h-screen bg-black text-white relative overflow-hidden flex flex-col">
      ...
      <div className="relative p-4 flex justify-between items-start z-20">
        ...
        <div className="flex flex-col gap-5 items-center">
           ...
           {/* CORRECTION : Bouton Flash avec action toggleFlash */}
      ...
      <div className="mt-auto pb-10 z-20 bg-gradient-to-t from-black/80 to-transparent">
        {/* NOUVEAU : Modes TikTok (Publier, Créer, Story) */}
        <div className="flex justify-center gap-8 mb-8 overflow-x-auto no-scrollbar px-6 items-center">
          <button className="text-[14px] font-bold text-white/40 whitespace-nowrap">Photos</button>
          <button className="text-[14px] font-bold text-white whitespace-nowrap relative">
            Publier
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-white rounded-full"></div>
          </button>
          <button className="text-[14px] font-bold text-white/40 whitespace-nowrap">Créer</button>
          <button className="text-[14px] font-bold text-white/40 whitespace-nowrap">Story</button>
        </div>

        <div className="flex items-center justify-around px-4">
          <button onClick={onOpenEffects} className="flex flex-col items-center gap-1 active:scale-90 transition-transform">
            <div className="w-14 h-14 rounded-2xl border-2 border-white/20 overflow-hidden bg-black/40 backdrop-blur-md flex items-center justify-center shadow-lg">
              <Sparkles size={30} className="text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]"/>
            </div>
            <span className="text-[11px] font-bold shadow-sm">Effets</span>
          </button>

          <div className={`w-22 h-22 rounded-full border-[6px] ${isRecording ? 'border-red-500/30 scale-110' : 'border-white'} flex items-center justify-center p-2 transition-all shadow-xl`}>
            <button onClick={handleCapture} className={`w-full h-full bg-red-500 rounded-full active:scale-95 transition-all ${isRecording ? 'scale-60 rounded-xl' : ''}`} />
          </div>

          {/* STYLE AMÉLIORÉ : Bouton Téléverser (Galerie) */}
          <button className="flex flex-col items-center gap-1 active:scale-90 transition-transform">
            <div className="w-14 h-14 rounded-2xl border-2 border-white/20 overflow-hidden bg-black/40 backdrop-blur-md flex items-center justify-center shadow-lg p-1.5">
               <div className="w-full h-full bg-gradient-to-br from-purple-500 via-pink-500 to-orange-500 rounded-lg flex items-center justify-center border border-white/20">
                  <ImageIcon size={22} className="text-white drop-shadow-md"/>
               </div>
            </div>
            <span className="text-[11px] font-bold shadow-sm">Téléverser</span>
          </button>
        </div>
      </div>
    </div>
