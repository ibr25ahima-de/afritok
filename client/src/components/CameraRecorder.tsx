...
=======
 useEffect(()=>{if(!isRecording)return;const m=selectedDuration==='10 min'?600:selectedDuration==='60 s'?60:15;if(recordingTime>=m){mediaRecorderRef.current?.stop();setIsRecording(false)}},[recordingTime,isRecording,selectedDuration]);
 return <div className="h-screen bg-black text-white relative overflow-hidden flex flex-col"><video ref={videoRef} autoPlay playsInline muted className="absolute inset-0 w-full h-full object-cover opacity-0 pointer-events-none"/><AREngine videoRef={videoRef} activeEffect={activeAREffect} isRecording={isRecording}/>{isRecording&&<div className="absolute top-0 left-0 w-full h-1.5 bg-white/20 z-30"><div className="h-full bg-yellow-400" style={{width:`${(recordingTime/(selectedDuration==='10 min'?600:selectedDuration==='60 s'?60:15))*100}%`}}/></div>}{isTimerActive&&<div className="absolute inset-0 flex items-center justify-center z-40 bg-black/40"><div className="text-8xl font-bold">{timerCountdown}</div></div>}<div className="relative p-4 flex justify-between items-start z-20"><button onClick={onClose}><X size={28}/></button><button onClick={onOpenMusic} className="bg-black/30 px-4 py-2 rounded-full flex items-center gap-2"><Music size={16}/>Ajouter un son</button><div className="flex flex-col gap-5 items-center"><button onClick={()=>setFacingMode(f=>f==='user'?'environment':'user')}><RefreshCw size={22}/><span className="text-[10px]">Retourner</span></button><button onClick={()=>{const i=SPEED_OPTIONS.indexOf(activeSpeed);const n=SPEED_OPTIONS[(i+1)%SPEED_OPTIONS.length];setActiveSpeed(n);if(videoRef.current)videoRef.current.playbackRate=parseFloat(n.replace('x',''))}}><Gauge size={22}/><span className="text-[10px]">{activeSpeed}</span></button><button onClick={()=>setTimerValue(v=>v===0?3:v===3?5:10)}><Timer size={22}/><span className="text-[10px]">{timerValue?`${timerValue}s`:'Retardateur'}</span></button><button onClick={toggleFlash}>{flashEnabled?<Zap/>:<ZapOff/>}<span className="text-[10px]">Flash</span></button><button onClick={()=>toast.info("Enregistrez d'abord une vidéo pour accéder au montage")}><Layout size={22}/><span className="text-[10px]">Montage</span></button></div></div><div className={`absolute left-0 right-0 z-20 ${showAREffectsPanel?'bottom-[280px]':'bottom-[60px]'}`}><div className="flex justify-start gap-3 overflow-x-auto px-6 pb-2 snap-x snap-mandatory scrollbar-hide" role="tablist" aria-label="Choisir le type et la durée" style={{ scrollbarWidth: 'none' }}>{['PHOTO','15 s','60 s','10 min'].map(d=><button key={d} role="tab" aria-selected={selectedDuration===d} onClick={()=>setSelectedDuration(d)} className={`shrink-0 min-w-[88px] snap-center rounded-full border px-4 py-2 text-sm font-semibold transition-colors ${selectedDuration===d ? 'border-yellow-400 bg-yellow-400 text-black' : 'border-white/40 bg-black/50 text-white'}`}>{d === 'PHOTO' ? 'Photo' : `Vidéo ${d}`}</button>)}</div><div className="flex justify-center mt-3"><div className="w-20 h-20 rounded-full border-[5px] border-white p-1.5"><button onClick={handleCapture} className="w-full h-full bg-red-500 rounded-full"/></div></div></div>{showAREffectsPanel&&<div className="absolute bottom-[50px] left-0 right-0 z-40"><EffectsPanel selectedEffect={activeAREffect} onSelectEffect={handleAREffectSelect}/></div>}<div className="absolute bottom-0 left-0 right-0 z-30 bg-black flex items-center px-4 h-[50px]"><div className="w-9 h-9 rounded-lg bg-gradient-to-br from-yellow-500 via-red-500 to-purple-600"/><button onClick={()=>setShowAREffectsPanel(v=>!v)} className="flex-1 mx-4 py-2.5 rounded-full font-bold"><Palette size={16}/>Effets</button><div className="flex gap-3"><LiveEntryButton className="px-3 py-2 text-sm"/><button onClick={()=>onPublish?.()}>PUBLIER</button><button onClick={()=>onOpenMusic?.()}>CRÉER</button></div></div></div>;
