...
export const CameraRecorder:React.FC<CameraRecorderProps>=({onVideoRecorded,onPhotoTaken,onClose,onOpenMusic,onPublish,selectedMusic})=>{
 ...
 return <div className="h-screen bg-black text-white relative overflow-hidden flex flex-col">
   ...
   <div className={`absolute left-0 right-0 z-20 ${showAREffectsPanel?'bottom-[280px]':'bottom-[60px]'}`}><div className="flex justify-center gap-4">{['10 min','60 s','15 s','PHOTO'].map(d=><button key={d} onClick={()=>setSelectedDuration(d)}>{d}</button>)}</div><div className="flex justify-center mt-3"><div className="w-20 h-20 rounded-full border-[5px] border-white p-1.5"><button onClick={handleCapture} className="w-full h-full bg-red-500 rounded-full"/></div></div></div>
