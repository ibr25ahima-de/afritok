...
export const CameraRecorder:React.FC<CameraRecorderProps>=({onVideoRecorded,onPhotoTaken,onClose,onOpenMusic,onPublish,selectedMusic})=>{
 ...
 const handleCapture=async()=>{if(timerValue>0&&!isRecording)await startTimer(timerValue);if(selectedDuration==='PHOTO'){takeProcessedPhoto();return}if(isRecording){stopRecording();return}const processedCanvas=processedCanvasRef.current;if(!processedCanvas||processedCanvas.width<2||processedCanvas.height<2){toast.error("La caméra avec effets n'est pas encore prête");return}if(!streamRef.current){toast.error("Caméra indisponible");return}
   const canvasStream=processedCanvas.captureStream(30);recordingCanvasStreamRef.current=canvasStream;const ac=new AudioContext();audioContextRef.current=ac;const d=ac.createMediaStreamDestination();const micSource=ac.createMediaStreamSource(streamRef.current);micSource.connect(d);let me:HTMLAudioElement|null=null;if(selectedMusic){me=new Audio(selectedMusic.url);me.crossOrigin='anonymous';musicElementRef.current=me;const ms=ac.createMediaElementSource(me);ms.connect(d);ms.connect(ac.destination)}const mixed=new MediaStream();canvasStream.getVideoTracks().forEach(t=>mixed.addTrack(t));d.stream.getAudioTracks().forEach(t=>mixed.addTrack(t));const chunks:Blob[]=[];const opt=MediaRecorder.isTypeSupported('video/webm;codecs=vp8,opus')?{mimeType:'video/webm;codecs=vp8,opus'}:{mimeType:'video/webm'};const mr=new MediaRecorder(mixed,opt);mr.ondataavailable=e=>{if(e.data.size>0)chunks.push(e.data)};mr.onstop=()=>{musicElementRef.current?.pause();audioContextRef.current?.close();recordingCanvasStreamRef.current?.getTracks().forEach(t=>t.stop());recordingCanvasStreamRef.current=null;const duration=Math.max(0,Math.round((performance.now()-recordingStartedAtRef.current)/1000));onVideoRecorded?.(new Blob(chunks,{type:mr.mimeType||'video/webm'}),duration)};mr.onerror=()=>{toast.error("Impossible d'enregistrer la vidéo avec l'effet");setIsRecording(false)};mr.start(1000);mediaRecorderRef.current=mr;recordingStartedAtRef.current=performance.now();if(me){await ac.resume();me.currentTime=0;me.playbackRate=parseFloat(activeSpeed.replace('x',''));me.play().catch(()=>{})}if(videoRef.current)videoRef.current.playbackRate=parseFloat(activeSpeed.replace('x',''));setIsRecording(true);setRecordingTime(0)};
 ...
 return <div className="h-screen bg-black text-white relative overflow-hidden flex flex-col">
   ...
   <AREngine videoRef={videoRef} activeEffect={activeAREffect} isRecording={isRecording} canvasRef={processedCanvasRef} onStatusChange={handleARStatus}/>
