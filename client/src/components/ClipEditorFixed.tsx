...
export function ClipEditorFixed({ src, duration, trimStart, trimEnd, cuts, onTrimChange, onCutsChange, onCurrentTimeChange, onClose }: Props) {
  ...
  const points = useMemo(() => [trimStart, ...splits.filter((p) => p > trimStart + 0.05 && p < end - 0.05), end].sort((a, b) => a - b), [trimStart, end, splits]);
  const clips = useMemo(() => {
    const out: Range[] = [];
  ...
  const save = () => {
    setHistory((h) => [...h.slice(-19), { cuts: cuts.map((x) => ({ ...x })), splits: [...splits], start: trimStart, end }]);
    setFuture([]);
  };
  ...
  useEffect(() => {
    ...
    const wait = () => new Promise<void>((resolve, reject) => {
      ...
      const ok = () => { clean(); resolve(); }, bad = () => { clean(); reject(new Error()); };
    ...
    const seek = (t: number) => new Promise<void>((resolve) => {
      let done = false;
      const f = () => { if (done) return; done = true; v.removeEventListener("seeked", f); resolve(); };
      v.addEventListener("seeked", f, { once: true }); v.currentTime = t; setTimeout(f, 600);
    });
    (async () => {
      try {
        await wait();
        ...
        const out: string[] = [], total = v.duration || duration;
        for (let i = 0; i < 18; i++) { if (cancelled) return; await seek(total * i / 17); ctx.drawImage(v, 0, 0, c.width, c.height); out.push(c.toDataURL("image/jpeg", 0.58)); }
  ...
  const splitHere = () => {
    ...
    if (!clip || time <= clip.start + 0.08 || time >= clip.end - 0.08) return toast.info("Déplace la ligne blanche exactement à l'endroit de la coupe");
  ...
  const deleteSelected = () => {
    ...
    save(); onCutsChange([...cuts, { start: clip.start, end: clip.end }]);
    ...
    setSelected(next); go(next?.start ?? trimStart); toast.success("Partie supprimée");
  };

  const deleteBefore = () => {
    if (time <= trimStart + 0.05) return toast.info("Déplace la ligne blanche vers la droite");
    save(); onTrimChange(time, end); onCutsChange(cuts.filter((c) => c.end > time)); setSplits((p) => p.filter((x) => x > time + 0.05)); go(time); toast.success("Partie avant supprimée");
  };
  const deleteAfter = () => {
    if (time >= end - 0.05) return toast.info("Déplace la ligne blanche vers la gauche");
    save(); onTrimChange(trimStart, time); onCutsChange(cuts.filter((c) => c.start < time)); setSplits((p) => p.filter((x) => x < time - 0.05)); go(Math.max(trimStart, time - 0.02)); toast.success("Partie après supprimée");
  };

  const undo = () => {
    const h = history.at(-1); if (!h) return;
    setFuture((f) => [...f, { cuts: cuts.map((x) => ({ ...x })), splits: [...splits], start: trimStart, end }]);
    setHistory((hs) => hs.slice(0, -1)); onCutsChange(h.cuts); onTrimChange(h.start, h.end); setSplits(h.splits); setSelected(null); go(h.start);
  };
  const redo = () => {
    const h = future.at(-1); if (!h) return;
    setHistory((hs) => [...hs, { cuts: cuts.map((x) => ({ ...x })), splits: [...splits], start: trimStart, end }]);
    setFuture((fs) => fs.slice(0, -1)); onCutsChange(h.cuts); onTrimChange(h.start, h.end); setSplits(h.splits); setSelected(null); go(h.start);
  };
        ...
        <div className="pb-3">
          <div
            ...
            onPointerDown={(e) => {
              ...
              const t = fromX(e.clientX); const clip = clips.find((c) => t >= c.start && t < c.end);
              setSelected(clip || null); beginDrag(e, "cursor");
            }}
            onPointerUp={(e) => {
              if (pointerRef.current === e.pointerId) { e.currentTarget.releasePointerCapture?.(e.pointerId); dragRef.current = null; pointerRef.current = null; }
            }}
            onPointerCancel={() => { dragRef.current = null; pointerRef.current = null; }}
          >
            <div className="absolute inset-0 flex pointer-events-none">
              {(thumbs.length ? thumbs : Array.from({ length: 18 })).map((t, i) => <div key={i} className="flex-1 min-w-0 border-r border-black/25">{t && <img src={t} alt="" draggable={false} className="w-full h-full object-cover" />}</div>)}
            </div>
            ...
            {clips.map((c, i) => { const is = selected && Math.abs(selected.start - c.start) < 0.03 && Math.abs(selected.end - c.end) < 0.03; return <div key={i} className={`absolute inset-y-0 border-2 pointer-events-none ${is ? "border-white" : "border-transparent"}`} style={{ left: `${c.start / duration * 100}%`, width: `${(c.end - c.start) / duration * 100}%` }} />; })}
            ...
            <button type="button" aria-label="Début de la vidéo" className="absolute top-0 bottom-0 w-5 -translate-x-1/2 z-50 bg-white rounded-l-md cursor-ew-resize" style={{ left: `${left}%`, touchAction: "none" }} onPointerDown={(e) => beginDrag(e, "start")} onPointerUp={(e) => { e.currentTarget.releasePointerCapture?.(e.pointerId); dragRef.current = null; pointerRef.current = null; }} />
            <button type="button" aria-label="Fin de la vidéo" className="absolute top-0 bottom-0 w-5 -translate-x-1/2 z-50 bg-white rounded-r-md cursor-ew-resize" style={{ left: `${right}%`, touchAction: "none" }} onPointerDown={(e) => beginDrag(e, "end")} onPointerUp={(e) => { e.currentTarget.releasePointerCapture?.(e.pointerId); dragRef.current = null; pointerRef.current = null; }} />
          </div>
...
function activeCuts(cuts: Range[], duration: number) {
  return cuts.map((c) => ({ start: Math.max(0, c.start), end: Math.min(duration, c.end) })).filter((c) => c.end > c.start + 0.02);
}
