...
export function ClipEditorFixed({ src, duration, trimStart, trimEnd, cuts, onTrimChange, onCutsChange, onCurrentTimeChange, onClose }: Props) {
  ...
  const points = useMemo(() => {
    const values = [trimStart, ...splitPoints.filter(p => p > trimStart + 0.05 && p < end - 0.05), end].sort((a, b) => a - b);
  ...
  const clips = useMemo(() => {
    ...
    for (let i = 0; i < points.length - 1; i++) {
      ...
      if (!activeCuts.some(c => start >= c.start - 0.02 && finish <= c.end + 0.02)) result.push({ start, end: finish });
    }
    return result;
  }, [points, activeCuts]);

  const snapshot = () => ({ cuts: cuts.map(c => ({ ...c })), splits: [...splitPoints], start: trimStart, end });
  const remember = () => { setHistory(h => [...h.slice(-19), snapshot()]); setFuture([]); };
  ...
  const moveDrag = (clientX: number) => {
    if (!dragMode) return;
    const value = timeFromClientX(clientX);
    if (dragMode === "playhead") setVideoTime(value);
  ...
  useEffect(() => {
    ...
    if (!video) return;
    const loaded = () => { video.currentTime = Math.max(0, trimStart); };
    const tick = () => {
      const current = video.currentTime;
      const removed = activeCuts.find(c => current >= c.start && current < c.end);
      if (removed) { video.currentTime = Math.min(removed.end + 0.001, end); return; }
      if (current >= end - 0.03) { video.pause(); setPlaying(false); return; }
      if (current >= trimStart) { setTime(current); onCurrentTimeChange?.(current); }
    };
    ...
    return () => { video.removeEventListener("loadedmetadata", loaded); video.removeEventListener("timeupdate", tick); video.removeEventListener("play", onPlay); video.removeEventListener("pause", onPause); };
  ...
  useEffect(() => {
    ...
    const wait = () => new Promise<void>((resolve, reject) => {
      ...
      const ok = () => { cleanup(); resolve(); }, bad = () => { cleanup(); reject(new Error("video")); };
    ...
    const seek = (t: number) => new Promise<void>(resolve => {
      ...
      video.addEventListener("seeked", finish, { once: true }); video.currentTime = t; window.setTimeout(finish, 700);
    });
    const make = async () => {
      try {
        await wait();
        ...
        const total = video.duration || duration; const result: string[] = [];
        for (let i = 0; i < 18; i++) { if (cancelled) return; await seek(total * i / 17); ctx.drawImage(video, 0, 0, canvas.width, canvas.height); result.push(canvas.toDataURL("image/jpeg", 0.58)); }
  ...
  const splitHere = () => {
    ...
    if (!clip || time <= clip.start + 0.08 || time >= clip.end - 0.08) {
      toast.info("Déplace la ligne blanche exactement à l'endroit de la coupe"); return;
    }
    if (splitPoints.some(p => Math.abs(p - time) < 0.08)) { toast.info("La vidéo est déjà divisée ici"); return; }
    remember();
  ...
  const deleteSelected = () => {
    ...
    if (!clip) { toast.info("Sélectionne d'abord la partie à supprimer"); return; }
  ...
  const deleteBefore = () => {
    if (time <= trimStart + 0.05) { toast.info("Déplace la ligne blanche vers la droite"); return; }
    remember();
  ...
  const deleteAfter = () => {
    if (time >= end - 0.05) { toast.info("Déplace la ligne blanche vers la gauche"); return; }
    remember();
  ...
  const undo = () => {
    ...
    if (!previous) return;
    setFuture(f => [...f, snapshot()]); setHistory(h => h.slice(0, -1));
    onCutsChange(previous.cuts); onTrimChange(previous.start, previous.end); setSplitPoints(previous.splits); setSelectedClip(null); setVideoTime(previous.start);
  };

  const redo = () => {
    const next = future[future.length - 1];
    if (!next) return;
    setHistory(h => [...h, snapshot()]); setFuture(f => f.slice(0, -1));
    onCutsChange(next.cuts); onTrimChange(next.start, next.end); setSplitPoints(next.splits); setSelectedClip(null); setVideoTime(next.start);
  };
      ...
      <div className="flex-1 min-h-0 flex flex-col justify-center">
        ...
        <div className="shrink-0 px-3">
          ...
          <div className="pb-3">
            <div
              ref={timelineRef}
              className="relative h-[76px] rounded-md select-none bg-white/10 overflow-visible"
            ...
            >
              <div className="absolute inset-0 flex pointer-events-none">
                {(thumbs.length ? thumbs : Array.from({ length: 18 })).map((thumb, i) => (
                  <div key={i} className="flex-1 min-w-0 border-r border-black/25">{thumb && <img src={thumb} alt="" draggable={false} className="w-full h-full object-cover" />}</div>
                ))}
              </div>

              {activeCuts.map((cut, i) => <div key={`${cut.start}-${cut.end}-${i}`} className="absolute inset-y-0 bg-black/75 pointer-events-none" style={{ left: `${cut.start / duration * 100}%`, width: `${(cut.end - cut.start) / duration * 100}%` }} />)}
              {splitPoints.map(p => <div key={p} className="absolute top-0 bottom-0 w-[2px] bg-white/90 z-20 pointer-events-none" style={{ left: `${p / duration * 100}%` }} />)}

              <div className="absolute top-0 bottom-0 w-[3px] bg-white z-30 pointer-events-none" style={{ left: `${playhead}%` }} />
              <div className="absolute -top-1 -translate-x-1/2 w-4 h-4 rounded-full bg-white z-40 pointer-events-none" style={{ left: `${playhead}%` }} />

              <button type="button" aria-label="Poignée début" className="absolute top-0 bottom-0 w-5 -translate-x-1/2 z-50 bg-white rounded-l-md cursor-ew-resize" style={{ left: `${leftTrim}%`, touchAction: "none" }} onPointerDown={(e) => { e.stopPropagation(); e.currentTarget.setPointerCapture?.(e.pointerId); setDragMode("start"); }} onPointerMove={(e) => { if (e.currentTarget.hasPointerCapture?.(e.pointerId)) moveDrag(e.clientX); }} onPointerUp={() => setDragMode(null)} />
              <button type="button" aria-label="Poignée fin" className="absolute top-0 bottom-0 w-5 -translate-x-1/2 z-50 bg-white rounded-r-md cursor-ew-resize" style={{ left: `${rightTrim}%`, touchAction: "none" }} onPointerDown={(e) => { e.stopPropagation(); e.currentTarget.setPointerCapture?.(e.pointerId); setDragMode("end"); }} onPointerMove={(e) => { if (e.currentTarget.hasPointerCapture?.(e.pointerId)) moveDrag(e.clientX); }} onPointerUp={() => setDragMode(null)} />

              {clips.map((clip, index) => {
                const selected = selectedClip && Math.abs(selectedClip.start - clip.start) < 0.03 && Math.abs(selectedClip.end - clip.end) < 0.03;
                return <button key={`${clip.start}-${clip.end}-${index}`} type="button" aria-label={`Clip ${index + 1}`} className={`absolute top-0 bottom-0 z-10 border-2 ${selected ? "border-white" : "border-transparent"}`} style={{ left: `${clip.start / duration * 100}%`, width: `${(clip.end - clip.start) / duration * 100}%`, touchAction: "none" }} onPointerDown={(e) => { e.stopPropagation(); setSelectedClip(clip); setVideoTime(clip.start); }} />;
              })}
            </div>
