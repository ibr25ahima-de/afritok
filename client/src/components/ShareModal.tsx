...
export default function ShareModal({ videoId, videoTitle, videoUrl, isOpen, onClose }: ShareModalProps) {
  ...
  const handleShare = async (platform: string) => {
    ...
    // 1. Enregistrer le partage en base de données via tRPC
    try {
      const res = await createShareMutation.mutateAsync({ videoId });
      if (res?.earning?.success) {
        console.log("💰 Gain partage enregistré !");
      }
    } catch (error) {
      console.error("Share tracking error", error);
    }

    // 2. Ouvrir la plateforme de partage
    switch (platform) {
      case "whatsapp":
