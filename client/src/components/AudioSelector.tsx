...
export default function AudioSelector({ onSelectAudio, onClose }: AudioSelectorProps) {
  ...
  const [tab, setTab] = useState<"popular" | "forYou" | "favorites" | "recent">("popular");

  const { data: sounds = [] } = trpc.music.getByTab.useQuery({
    tab,
  });
  ...
  const handlePlayPreview = (sound: any) => {
    if (audioPlayerRef.current && isPlaying) {
      audioPlayerRef.current.stop();
      setIsPlaying(null);
    }
    audioPlayerRef.current = new AudioPlayer(sound.audioUrl || sound.url);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 border border-purple-800/50 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex border-b border-purple-800/30 overflow-x-auto">
          <button
