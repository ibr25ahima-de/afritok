...
export function ClipEditorFunctional({src,duration,trimStart,trimEnd,cuts,onTrimChange,onCutsChange,onCurrentTimeChange,onClose}:Props){
 ...
 const pointerStart=(e:React.PointerEvent<HTMLDivElement>)=>{if(!selecting)return;e.preventDefault();dragRef.current=true;try{e.currentTarget.setPointerCapture?.(e.pointerId)}catch{}vref.current?.pause();move(e.clientX)};
 ...
 const touchStart=(e:React.TouchEvent<HTMLDivElement>)=>{if(!selecting)return;const touch=e.touches[0];if(!touch)return;dragRef.current=true;vref.current?.pause();move(touch.clientX)};
