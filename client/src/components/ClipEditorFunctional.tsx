...
export function ClipEditorFunctional({src,duration,trimStart,trimEnd,cuts,onTrimChange,onCutsChange,onCurrentTimeChange,onClose}:Props){
 ...
 const move=(x:number)=>{const el=tref.current;if(!el||end<=start)return;const r=el.getBoundingClientRect();const ratio=Math.max(0,Math.min(1,(x-r.left)/Math.max(1,r.width)));seek(start+ratio*(end-start));};
 const pointerStart=(e:React.PointerEvent<HTMLDivElement>)=>{if(e.pointerType!=="touch"&&e.button!==0)return;e.preventDefault();dragRef.current=true;e.currentTarget.setPointerCapture?.(e.pointerId);vref.current?.pause();move(e.clientX)};
