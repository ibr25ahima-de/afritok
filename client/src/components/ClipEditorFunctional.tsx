...
export function ClipEditorFunctional({src,duration,trimStart,trimEnd,cuts,onTrimChange,onCutsChange,onCurrentTimeChange,onClose}:Props){
 ...
 const [d,setD]=useState(duration>0?duration:0),[t,setT]=useState(trimStart||0),[playing,setPlaying]=useState(false),[splits,setSplits]=useState<number[]>([]),[thumbs,setThumbs]=useState<string[]>([]),[history,setHistory]=useState<any[]>([]),[future,setFuture]=useState<any[]>([]),[ready,setReady]=useState(false);
 ...
 const pointerStart=(e:React.PointerEvent<HTMLDivElement>)=>{e.preventDefault();dragRef.current=true;try{e.currentTarget.setPointerCapture?.(e.pointerId)}catch{}vref.current?.pause();move(e.clientX)};
 const pointerMove=(e:React.PointerEvent<HTMLDivElement>)=>{if(dragRef.current){e.preventDefault();move(e.clientX)}};
 const pointerEnd=(e:React.PointerEvent<HTMLDivElement>)=>{dragRef.current=false;try{e.currentTarget.releasePointerCapture?.(e.pointerId)}catch{}};
 const touchStart=(e:React.TouchEvent<HTMLDivElement>)=>{const touch=e.touches[0];if(!touch)return;dragRef.current=true;vref.current?.pause();move(touch.clientX)};
 const touchMove=(e:React.TouchEvent<HTMLDivElement>)=>{const touch=e.touches[0];if(!touch||!dragRef.current)return;e.preventDefault();move(touch.clientX)};
 const touchEnd=()=>{dragRef.current=false};
 ...
 return <div className="fixed inset-0 z-[110] bg-black text-white flex flex-col overflow-hidden"><header className="h-14 shrink-0 flex items-center justify-between px-3"><button onClick={onClose} className="w-10 h-10 flex items-center justify-center"><ArrowLeft size={26}/></button><b className="text-sm">Modifier la vidéo</b><button onClick={onClose} className="w-10 h-10 flex items-center justify-center text-red-400"><Check size={27}/></button></header><div className="flex-1 min-h-0 flex flex-col"><div className="flex-1 min-h-0 flex items-center justify-center relative px-2 py-2"><video ref={vref} src={src} muted playsInline preload="auto" controls={false} className="w-full h-full max-h-full object-contain" onClick={toggle}/>{!playing&&<button aria-label={ready?"Lire la vidéo":"Chargement de la vidéo"} onClick={toggle} disabled={!ready} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-30 h-16 w-16 rounded-full bg-black/60 flex items-center justify-center disabled:opacity-40"><Play size={30} fill="white"/></button>}</div><div className="shrink-0 px-3"><div className="h-10 flex items-center justify-center gap-6"><span className="tabular-nums text-sm">{fmt(t-start)} / {fmt(end-start)}</span><button aria-label={playing?"Pause":"Lire"} onClick={toggle} disabled={!ready} className="disabled:opacity-40">{playing?<Pause size={21}/>:<Play size={21} fill="white"/>}</button><button onClick={undo} disabled={!history.length} className="disabled:opacity-25"><Undo2 size={20}/></button><button onClick={redo} disabled={!future.length} className="disabled:opacity-25"><Redo2 size={20}/></button></div><div className="pb-3"><div ref={tref} className="relative h-[84px] rounded-md overflow-hidden bg-white/10 select-none cursor-ew-resize" style={{touchAction:"none"}} onPointerDown={pointerStart} onPointerMove={pointerMove} onPointerUp={pointerEnd} onPointerCancel={pointerEnd} onTouchStart={touchStart} onTouchMove={touchMove} onTouchEnd={touchEnd} onTouchCancel={touchEnd}><div className="flex h-16 pointer-events-none">{(thumbs.length?thumbs:Array.from({length:18})).map((f,i)=><div key={i} className="flex-1 min-w-0 border-r border-black/30 bg-white/5">{f&&<img src={f} alt="" draggable={false} className="w-full h-16 object-cover"/>}</div>)}</div>{removed.map((r,i)=><div key={i} className="absolute inset-y-0 bg-black/75 pointer-events-none z-10" style={{left:`${(r.start-start)/Math.max(.001,end-start)*100}%`,width:`${(r.end-r.start)/Math.max(.001,end-start)*100}%`}}/>)}{splits.map(x=><div key={x} className="absolute inset-y-0 w-[2px] bg-white/80 pointer-events-none z-30" style={{left:`${(x-start)/Math.max(.001,end-start)*100}%`}}/>)}<div className="absolute top-0 bottom-0 w-[3px] bg-white z-40 pointer-events-none" style={{left:`${Math.max(0,Math.min(100,pct))}%`}}/><div className="absolute top-0 -translate-x-1/2 w-5 h-5 rounded-full bg-white z-50 pointer-events-none" style={{left:`${Math.max(0,Math.min(100,pct))}%`}}/></div><div className="flex justify-between text-[11px] text-white/60 mt-1"><span>00:00</span><span>{fmt(end-start)}</span></div></div></div></div><div className="shrink-0 border-t border-white/10 bg-black px-2 py-3 pb-[max(12px,env(safe-area-inset-bottom))]"><div className="flex gap-2 overflow-x-auto px-2"><button onClick={front} className="shrink-0 rounded-xl bg-white/10 px-3 py-2 text-xs font-semibold">Début</button><button onClick={back} className="shrink-0 rounded-xl bg-white/10 px-3 py-2 text-xs font-semibold">Fin</button><button onClick={split} className="shrink-0 rounded-xl bg-white/10 px-3 py-2 text-xs font-semibold flex items-center gap-1"><Scissors size={15}/>Diviser</button><button onClick={del} className="shrink-0 rounded-xl bg-red-500/90 px-3 py-2 text-xs font-semibold flex items-center gap-1"><Trash2 size={15}/>Supprimer</button></div><p className="text-[10px] text-white/45 text-center mt-2">Fais glisser la ligne blanche avec le doigt • la vidéo suit • Diviser puis Supprimer</p></div></div>;
}
