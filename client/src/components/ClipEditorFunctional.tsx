...
import { ArrowLeft, Check, Play, Pause, Undo2, Redo2, Scissors, Trash2, Gauge } from "lucide-react";
...
export function ClipEditorFunctional({src,duration,trimStart,trimEnd,cuts,onTrimChange,onCutsChange,onCurrentTimeChange,onClose}:Props){
 ...
 const [d,setD]=useState(duration>0?duration:0),[t,setT]=useState(trimStart||0),[playing,setPlaying]=useState(false),[splits,setSplits]=useState<number[]>([]),[thumbs,setThumbs]=useState<string[]>([]),[history,setHistory]=useState<any[]>([]),[future,setFuture]=useState<any[]>([]),[ready,setReady]=useState(false),[selecting,setSelecting]=useState(false),[speed,setSpeed]=useState(1),[exporting,setExporting]=useState(false);
 ...
 useEffect(()=>{const v=vref.current;if(!v)return;let disposed=false;const fixDuration=()=>new Promise<number>((resolve)=>{if(Number.isFinite(v.duration)&&v.duration>0){resolve(v.duration);return}if(duration>0){resolve(duration);return}let settled=false;const finish=(value:number)=>{if(settled)return;settled=true;v.removeEventListener("durationchange",onChange);resolve(value)};const onChange=()=>{const dur=v.duration;if(Number.isFinite(dur)&&dur>0)finish(dur)};v.addEventListener("durationchange",onChange);setTimeout(()=>finish(Number.isFinite(v.duration)&&v.duration>0?v.duration:0),1200)});const readyFn=async()=>{let dur=v.duration;if(!Number.isFinite(dur)||dur<=0)dur=await fixDuration();if(disposed||!Number.isFinite(dur)||dur<=0)return;setD(dur);setReady(true);if(trimEnd<=0)onTrimChange(trimStart||0,dur);const initial=Math.min(Math.max(tRef.current||trimStart||0,trimStart||0),trimEnd>0?trimEnd:dur);try{v.currentTime=initial}catch{};tRef.current=initial;setT(initial)};const time=()=>{const x=v.currentTime;const cut=removed.find(r=>x>=r.start&&x<r.end);if(cut){try{v.currentTime=Math.min(cut.end+.001,end)}catch{};return}if(end>start&&x>=end-.03){v.pause();setPlaying(false);seek(end,false);return}if(!dragRef.current){tRef.current=x;setT(x)}};const play=()=>setPlaying(true),pause=()=>setPlaying(false),err=()=>{setReady(false);toast.error("La vidéo ne peut pas être lue dans le montage")};v.addEventListener("loadedmetadata",readyFn);v.addEventListener("loadeddata",readyFn);v.addEventListener("canplay",readyFn);v.addEventListener("timeupdate",time);v.addEventListener("play",play);v.addEventListener("pause",pause);v.addEventListener("error",err);if(v.readyState>=1)readyFn();return()=>{disposed=true;v.removeEventListener("loadedmetadata",readyFn);v.removeEventListener("loadeddata",readyFn);v.removeEventListener("canplay",readyFn);v.removeEventListener("timeupdate",time);v.removeEventListener("play",play);v.removeEventListener("pause",pause);v.removeEventListener("error",err)}},[src,end,removed,trimEnd,trimStart,onTrimChange]);
 useEffect(() => {
  let stop = false;
 ...
 const cycleSpeed=()=>setSpeed(s=>s===1?2:s===2?3:s===3?.5:1);
 const applySpeed=async():Promise<boolean>=>{
   ...
   try{
     ...
     v.currentTime=start;
     await new Promise<void>(resolve=>{let done=false;const finish=()=>{if(done)return;done=true;v.removeEventListener("seeked",finish);resolve()};v.addEventListener("seeked",finish,{once:true});setTimeout(finish,600)});
     v.playbackRate=speed;recorder.start(250);await v.play();
     await new Promise<void>(resolve=>{
       const tick=()=>{
         if(v.currentTime>=end||v.ended){resolve();return}
         ctx.drawImage(v,0,0,canvas.width,canvas.height);requestAnimationFrame(tick);
       };
       tick();
     });
     v.pause();v.playbackRate=1;recorder.stop();
     const blob=await stopped;if(blob.size<1024)throw new Error("Export vide");
     setFile(new File([blob],"afritok-vitesse.webm",{type:blob.type||"video/webm"}));
     toast.success(`Vitesse ${speed}x appliquée`);
     return true;
   }catch(error){console.error("[ClipEditor] speed",error);toast.error("Impossible d'appliquer la vitesse sur cet appareil");return false}
   finally{if(vref.current)vref.current.playbackRate=1;setExporting(false)}
 };
 const toggle=async()=>{
