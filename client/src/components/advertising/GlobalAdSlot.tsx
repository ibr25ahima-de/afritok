...
/**
 * Emplacement publicitaire global.
    ...
    <div className="fixed bottom-20 left-3 right-3 z-[80] mx-auto max-w-lg">
      <div className="overflow-hidden rounded-2xl border border-amber-400/40 bg-black/95 shadow-2xl backdrop-blur-md">
        <div className="flex items-center gap-2 border-b border-white/10 px-3 py-2">
          ...
          <button type="button" onClick={() => setClosed(true)} className="ml-auto rounded-full p-1 text-gray-400 hover:bg-white/10 hover:text-white" aria-label="Fermer la publicité">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex gap-3 p-3">
          {ad.adType === "image" && ad.imageUrl ? (
            <img src={ad.imageUrl} alt={ad.name} className="h-20 w-20 shrink-0 rounded-xl object-cover" />
          ) : null}
          {ad.adType === "video" && ad.videoUrl ? (
            <video src={ad.videoUrl} className="h-20 w-20 shrink-0 rounded-xl object-cover" muted playsInline />
          ) : null}
          <div className="min-w-0 flex-1">
            ...
            {ad.destinationUrl ? (
              <button type="button" onClick={handleClick} className="mt-2 inline-flex items-center gap-1 rounded-lg bg-amber-400 px-3 py-1.5 text-xs font-black text-black hover:bg-amber-300">
                Découvrir <ExternalLink size={13} />
              </button>
            ) : null}
          </div>
        </div>
