...
export const LM = {
  ...
  innerLips: [78, 95, 88, 178, 87, 14, 317, 402, 318, 324, 308, 415, 310, 311, 312, 13, 82, 81, 42, 183],
  nose: [1, 2, 98, 327, 6, 197, 195, 5, 4],
...
export function getPoints(landmarks: NormalizedLandmark[], indices: number[], width: number, height: number): Point[] {
  return indices.filter((i) => landmarks[i]).map((i) => ({ x: landmarks[i].x * width, y: landmarks[i].y * height }));
}
...
export function getFaceGeometry(landmarks: NormalizedLandmark[], width: number, height: number): FaceGeometry {
  const points = getPoints(landmarks, LM.faceOval, width, height);
  if (!points.length) return { left: 0, right: width, top: 0, bottom: height, cx: width / 2, cy: height / 2, width, height };
  let left = width, right = 0, top = height, bottom = 0;
  for (const point of points) {
    left = Math.min(left, point.x); right = Math.max(right, point.x);
    top = Math.min(top, point.y); bottom = Math.max(bottom, point.y);
  }
  return { left, right, top, bottom, cx: (left + right) / 2, cy: (top + bottom) / 2, width: right - left, height: bottom - top };
}

export function buildPolygonMask(points: Point[], width: number, height: number): Uint8Array {
  ...
  for (let y = minY; y <= maxY; y++) {
    ...
    for (let i = 0, j = points.length - 1; i < points.length; j = i++) {
      ...
      if ((a.y > y) !== (b.y > y)) intersections.push(b.x + ((y - b.y) * (a.x - b.x)) / (a.y - b.y));
    }
