...
import { db } from "../db";
import { earnings, users, videos } from "../../drizzle/schema";
import { MONETIZATION } from "../monetization-config";
import { getUserById } from "./users";
import { getFollowerCount } from "./followers";
...
export async function createEarning(
  ...
  try {
    ...
    if (source === "creator_view") {
      // Logic from server/db.ts
      reward = MONETIZATION.rewards.creator_view || amount;
    } else {
      reward =
