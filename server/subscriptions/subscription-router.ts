...
import { confirmPayment } from "../payments/payment-service";
import { isPremiumSandboxEnabled } from "../payments/premium-sandbox";
...
export const subscriptionRouter = router({
  ...
  status: protectedProcedure.query(async ({ ctx }) => {
    const subscription = await getActivePremiumSubscription(ctx.user.id);
    return { isPremium: Boolean(subscription), expiresAt: subscription?.expires_at ?? null };
  }),
  analytics: protectedProcedure
    .input(z.object({ days: z.number().int().min(7).max(90).default(30) }))
    .query(async ({ ctx, input }) => getPremiumAnalytics(ctx.user.id, input.days)),
  createPayment: protectedProcedure
    .input(z.object({ planId: z.string(), operator: z.enum(PAYMENT_OPERATORS), phone: z.string().min(8) }))
    .mutation(async ({ ctx, input }) => {
      ...
      // End-to-end Premium test only. Disabled by default; never uses real money.
      // In production, activation happens only from a verified provider webhook.
      if (isPremiumSandboxEnabled() && provider.status === "success" && provider.providerReference) {
  ...
  paymentStatus: protectedProcedure
    ...
    .query(async ({ ctx, input }) => getPremiumPaymentStatus(ctx.user.id, input.referenceId)),
  applyVideoOptions: protectedProcedure
    .input(premiumVideoOptionsSchema)
    .mutation(async ({ ctx, input }) => applyPremiumVideoOptions(ctx.user.id, input.videoId, input)),
});
