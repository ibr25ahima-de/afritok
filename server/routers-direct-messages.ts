...
export const directMessagesRouter = router({
  ...
  getConversationMessages: protectedProcedure.input(z.object({ conversationId: z.number(), limit: z.number().default(50), offset: z.number().default(0) })).query(({ input }) => getDirectMessagesManager().getConversationMessages(input.conversationId, input.limit, input.offset)),
  ...
  markMessagesAsRead: protectedProcedure.input(z.object({ conversationId: z.number() })).mutation(async ({ ctx, input }) => ({ success: await getDirectMessagesManager().markMessagesAsRead(input.conversationId, ctx.user.id) })),
  ...
  getLastConversationMessage: protectedProcedure.input(z.object({ conversationId: z.number() })).query(({ input }) => getDirectMessagesManager().getLastConversationMessage(input.conversationId)),
  ...
  searchConversationMessages: protectedProcedure.input(z.object({ conversationId: z.number(), query: z.string(), limit: z.number().default(20) })).query(({ input }) => getDirectMessagesManager().searchConversationMessages(input.conversationId, input.query, input.limit)),
  getConversationStats: protectedProcedure.input(z.object({ conversationId: z.number() })).query(({ input }) => getDirectMessagesManager().getConversationStats(input.conversationId)),
});
