...
export interface LiveParticipant { userId: number; username: string; joinedAt: Date; role: LiveRole; isMuted: boolean; isVideoOff: boolean; peerId?: string; stageSlot?: number; }
export interface LiveSession { sessionId: string; hostId: number; hostUsername: string; title: string; description: string; type: LiveType; layout: LiveLayout; centerParticipantId?: number; state: LiveState; participants: Map<number, LiveParticipant>; maxParticipants: number; viewerCount: number; startedAt: Date; endedAt?: Date; isPublic: boolean; thumbnail?: string; recordingUrl?: string; rewardId?: string; giftRevenue: number; }
export class LiveSessionsManager {
  ...
  private nextFreeStageSlot(session: LiveSession): number | undefined {
    ...
    // Slot 0 is reserved for the host. maxParticipants is the number of guest places.
  ...
  addParticipant(sessionId: string, userId: number, username: string, role: 'guest' | 'viewer' = 'viewer'): boolean {
    const session = this.sessions.get(sessionId); if (!session || session.state === 'ended') return false; const existing = session.participants.get(userId);
    if (existing) { if (role === 'guest' && existing.role === 'viewer') { const slot = this.nextFreeStageSlot(session); if (slot === undefined) return false; existing.role = 'guest'; existing.stageSlot = slot; session.viewerCount = Math.max(0, session.viewerCount - 1); } return true; }
    const stageSlot = role === 'guest' ? this.nextFreeStageSlot(session) : undefined; if (role === 'guest' && stageSlot === undefined) return false;
    session.participants.set(userId, { userId, username, joinedAt: new Date(), role, isMuted: false, isVideoOff: false, stageSlot }); if (role === 'viewer') session.viewerCount++; return true;
  }
  removeParticipant(sessionId: string, userId: number): boolean { const session = this.sessions.get(sessionId); if (!session) return false; const participant = session.participants.get(userId); if (!participant) return false; if (participant.role === 'viewer') session.viewerCount = Math.max(0, session.viewerCount - 1); session.participants.delete(userId); this.userSessions.delete(userId); if (session.centerParticipantId === userId) session.centerParticipantId = session.hostId; if (participant.role === 'host') this.closeSession(sessionId); return true; }
  ...
  setParticipantRole(sessionId: string, userId: number, role: 'admin' | 'guest' | 'viewer'): boolean { const session = this.sessions.get(sessionId); if (!session) return false; const participant = session.participants.get(userId); if (!participant || participant.role === 'host') return false; const wasViewer = participant.role === 'viewer'; const willViewer = role === 'viewer'; if (!willViewer && (wasViewer || participant.stageSlot === undefined)) { const slot = participant.stageSlot ?? this.nextFreeStageSlot(session); if (slot === undefined) return false; participant.stageSlot = slot; } if (wasViewer && !willViewer) session.viewerCount = Math.max(0, session.viewerCount - 1); if (!wasViewer && willViewer) { session.viewerCount++; participant.stageSlot = undefined; if (session.centerParticipantId === userId) session.centerParticipantId = session.hostId; } participant.role = role; return true; }
  setCenterParticipant(sessionId: string, userId: number): boolean { const session = this.sessions.get(sessionId); if (!session) return false; const participant = session.participants.get(userId); if (!participant || (!isStageRole(participant.role) && participant.role !== 'host')) return false; session.centerParticipantId = userId; return true; }
  ...
  getSessionStats(sessionId: string) { const s = this.sessions.get(sessionId); if (!s) return null; const guests = Array.from(s.participants.values()).filter((p) => p.role === 'guest'); return { sessionId, title: s.title, hostUsername: s.hostUsername, type: s.type, layout: s.layout, state: s.state, duration: s.endedAt ? s.endedAt.getTime() - s.startedAt.getTime() : 0, participantCount: s.participants.size, guestCount: guests.length, viewerCount: s.viewerCount, maxParticipants: s.maxParticipants, giftRevenue: s.giftRevenue }; }
}
