...
export async function getFeedVideos(limit = 20, offset = 0) {
  console.log("[getFeedVideos] START - limit:", limit, "offset:", offset);
  
  try {
    console.log("[getFeedVideos] Building query...");
    ...
    console.log("[getFeedVideos] SUCCESS - returned", result.length, "videos");
    console.log("[getFeedVideos] First video:", result[0] ? { id: result[0].id, title: result[0].title, videoUrl: result[0].videoUrl } : "NONE");
    
    return result;
  } catch (error) {
    console.error("[getFeedVideos] ERROR:", error);
    console.error("[getFeedVideos] Error stack:", (error as Error).stack);
    throw error;
  }
}

export async function getUserVideos(userId: number) {
  console.log("[getUserVideos] START - userId:", userId);
  
  try {
    const result = await db.select().from(videos).where(eq(videos.userId, userId)).orderBy(desc(videos.createdAt));
    console.log("[getUserVideos] SUCCESS - returned", result.length, "videos for user", userId);
    return result;
  } catch (error) {
    console.error("[getUserVideos] ERROR:", error);
    console.error("[getUserVideos] Error stack:", (error as Error).stack);
    throw error;
  }
}
