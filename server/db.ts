...
export async function createEarning(
  ...
  try {
    ...
    if (isCreator) {
      // 🚀 MODE STARTUP : tout le monde gagne (conditions désactivées)
    }
...
export async function createWithdrawal(userId: number, amount: number, method: string, details: any) {
  const user = await getUserById(userId);
  if (!user) return { success: false, reason: "user_not_found" };

  const balance = parseFloat(user.totalEarnings?.toString() || "0");
  if (balance < amount) return { success: false, reason: "insufficient_balance" };

  if (amount < MONETIZATION.withdrawal.minAmount) {
    return { success: false, reason: "min_amount_not_reached" };
  }
  ...
  if (pending.length > 0) {
    return { success: false, reason: "pending_withdrawal_exists" };
  }
  ...
  const newTotal = (balance - amount).toFixed(2);
  await db.update(users).set({ totalEarnings: newTotal }).where(eq(users.id, userId));
