...
export async function getFeedVideos(limit = 20, offset = 0) {
  return db
    .select({
      ...
      views: sql<number>`COALESCE(${videos.views}, 0)`,
      likes: sql<number>`COALESCE(${videos.likes}, 0)`,
      comments: sql<number>`COALESCE(${videos.comments}, 0)`,
      shares: sql<number>`COALESCE(${videos.shares}, 0)`,
      favorites: sql<number>`COALESCE(${videos.favorites}, 0)`,
...
export async function deleteComment(commentId: number, userId: number) {
  try {
    ...
    await db
      .update(videos)
      .set({ comments: sql`${videos.comments} - 1` })
      .where(eq(videos.id, existing[0].videoId));
