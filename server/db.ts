...
export async function getUserEarnings(userId: number) {
  try {
    ...
    // 2. Total des retraits (réussis ou en attente)
    ...
    // 3. Solde actuel
    const balance = Math.max(0, totalEarnings - totalWithdrawals);
    ...
    return { total: 0, withdrawn: 0, balance: 0 };
  }
}
...
export async function getPlatformStats() {
  const totalResult = await db.select({ total: sql<number>`SUM(${earnings.amount})` }).from(earnings);
  const transactionsResult = await db.select({ count: sql<number>`COUNT(*)` }).from(earnings);
  
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const todayResult = await db.select({ total: sql<number>`SUM(${earnings.amount})` }).from(earnings).where(gt(earnings.createdAt, today));

  return {
    total: Number(totalResult[0]?.total || 0),
    today: Number(todayResult[0]?.total || 0),
    transactions: Number(transactionsResult[0]?.count || 0),
  };
}

export async function fixDatabase() {
  try {
    // Vérifier si la table withdrawals existe, sinon la créer via SQL brut si nécessaire
    // Mais normalement drizzle-kit s'en occupe.
    console.log("Database fix check...");
  } catch (e) {
    console.error("fixDatabase error:", e);
  }
  }
