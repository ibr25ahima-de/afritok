...
export async function getFeedVideos(limit: number, offset: number) {
  try {
    ...
    console.error("❌ [getFeedVideos] Error fetching videos:", error);
    return [];
  }
}
...
export async function createWithdrawal(userId: number, amount: number, method: string, phone: string) {
  const user = await getUserById(userId);
  if (!user || user.totalEarnings < amount) return { success: false, reason: "insufficient_balance" };
...
export async function updateDisplaySettings(userId: number, settings: any) {
  await db
    .update(users)
    .set(settings)
    .where(eq(users.id, userId));
