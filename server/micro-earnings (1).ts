...
import { earnings, microEarnings } from './schema';
...
async function saveEarning(earning: MicroEarning) {
  ...
  try {
    // 1. Enregistrer dans micro_earnings (moteur principal)
    await db.insert(microEarnings).values({
      ...earning,
      amount: earning.amount.toString(),
    });

    // 2. Enregistrer dans earnings (affichage UI)
    await db.insert(earnings).values({
      userId: earning.userId,
      amount: earning.amount.toString(),
      source: earning.type,
      videoId: earning.videoId || null,
    });
...
export async function getUserBalance(userId: number): Promise<UserBalance> {
  ...
  try {
    ...
    const totalEarned = userEarnings.reduce((sum, e) => sum + Number(e.amount), 0);
