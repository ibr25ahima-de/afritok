...
export const financeRouter = router({
  ...
  getPlatformBalance: protectedProcedure.query(async ({ ctx }) => {
    ...
    // ============================================
    // 🔎 DEBUG : vérifier les revenus de la plateforme
    // ============================================

    const allPlatformEarnings = await db
      .select()
      .from(earnings)
      .where(eq(earnings.userId, ADMIN_ID));

    console.log("========== FINANCE AFritok ==========");
    console.log(
      "Nombre de transactions plateforme =",
      allPlatformEarnings.length
    );

    console.log(
      "Transactions plateforme =",
      allPlatformEarnings.slice(0, 20)
    );
    ...
    const earningsResult = await db
      .select({
        total: sql<string>`
          COALESCE(
            SUM(CAST(${earnings.amount} AS double precision)),
            0
          )
        `,
      })
