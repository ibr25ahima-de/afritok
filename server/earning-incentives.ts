...
import { recordChallengeEarning, getUserBalance } from "./micro-earnings";
...
Claim incentive
*/
export async function claimIncentive(incentive: Incentive): Promise<boolean> {
  try {
    if (incentive.status !== "available") return false;
    if (new Date() > incentive.expiresAt) return false;

    // 🔥 paiement réel
    await recordChallengeEarning(incentive.userId);
