...
export async function recordAdvertisingImpression(
  campaignId: number,
  userId?: number
) {
  const eventReference =
    `imp_${campaignId}_${Date.now()}_${Math.random()
      .toString(36)
      .slice(2, 10)}`;
  ...
  return await db.transaction(async (tx) => {
    ...
    if (campaign[0].status !== "active") {
      throw new Error(
        "Cette publicité n'est pas active."
      );
    }
...
export async function recordAdvertisingClick(
  campaignId: number,
  userId?: number
) {
  const eventReference =
    `click_${campaignId}_${Date.now()}_${Math.random()
      .toString(36)
      .slice(2, 10)}`;
