...
export const likeRouter = router({
  ...
  getMyLikedVideos: protectedProcedure.query(async ({ ctx }) => {
    return db.select({ video: videos }).from(likes).innerJoin(videos, eq(videos.id, likes.videoId)).where(eq(likes.userId, ctx.user.id));
  }),
});
