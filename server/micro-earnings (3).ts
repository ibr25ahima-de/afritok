/**
 * Micro-Earnings System for Afritok (FINAL CLEAN VERSION)
 */

import { getDb } from './db';
import { microEarnings, earnings } from './schema';
...
async function saveEarning(earning: MicroEarning) {
  ...
  try {
    ...
    // 2. Sauvegarde pour affichage UI
...
async function alreadyExists(userId: number, type: string, videoId?: number) {
  ...
  if (videoId) {
    return results.some((e) => e.videoId === videoId);
  }
