...
export const appRouter = router({
  ...
  video: router({
    feed: publicProcedure.input(z.object({ limit: z.number().default(20), offset: z.number().default(0) })).query(async ({ input }) => getFeedVideos(input.limit, input.offset)),
    ...
    getByUser: publicProcedure.input(z.object({ userId: z.number() })).query(async ({ input, ctx }) => { const [owner] = await db.select({ id: users.id, profilePublic: users.profilePublic }).from(users).where(eq(users.id, input.userId)).limit(1); if (!owner) throw new TRPCError({ code: "NOT_FOUND", message: "Utilisateur introuvable." }); if (!owner.profilePublic && ctx.user?.id !== owner.id && ctx.user?.role !== "admin") return []; return getUserVideos(input.userId); }),
    delete: protectedProcedure.input(z.object({ videoId: z.number() })).mutation(async ({ ctx, input }) => { const video = await getVideoById(input.videoId); if (!video) throw new TRPCError({ code: "NOT_FOUND", message: "Vidéo introuvable." }); if (video.userId !== ctx.user.id && ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN", message: "Vous ne pouvez supprimer que vos propres vidéos." }); try { await storageDeleteVideo(video.videoUrl); } catch (error) { console.error("[video.delete] Storage cleanup failed; deleting database record anyway:", error); } await db.delete(comments).where(eq(comments.videoId, input.videoId)); await db.delete(likes).where(eq(likes.videoId, input.videoId)); await db.delete(favorites).where(eq(favorites.videoId, input.videoId)); await db.delete(shares).where(eq(shares.videoId, input.videoId)); await db.delete(videos).where(eq(videos.id, input.videoId)); return { success: true, videoId: input.videoId }; }),
  }),
  like: likeRouter, comment: commentRouter, favorite: favoriteRouter, share: shareRouter, admin: adminRouter,
  follower: router({ toggle: protectedProcedure.input(z.object({ userId: z.number() })).mutation(async ({ ctx, input }) => { if (ctx.user.id === input.userId) throw new TRPCError({ code: "BAD_REQUEST", message: "Vous ne pouvez pas vous suivre vous-même." }); const following = await isFollowing(ctx.user.id, input.userId); if (following) { await db.delete(followers).where(and(eq(followers.followerId, ctx.user.id), eq(followers.followingId, input.userId))); return { following: false }; } const [target] = await db.select({ id: users.id, profilePublic: users.profilePublic }).from(users).where(eq(users.id, input.userId)).limit(1); if (!target) throw new TRPCError({ code: "NOT_FOUND", message: "Utilisateur introuvable." }); if (!target.profilePublic) throw new TRPCError({ code: "FORBIDDEN", message: "Ce profil est privé." }); await db.insert(followers).values({ followerId: ctx.user.id, followingId: input.userId }); return { following: true }; }), getCount: publicProcedure.input(z.object({ userId: z.number() })).query(async ({ input }) => ({ followers: await getFollowerCount(input.userId), following: await getFollowingCount(input.userId) })), isFollowing: protectedProcedure.input(z.object({ userId: z.number() })).query(async ({ ctx, input }) => ({ following: await isFollowing(ctx.user.id, input.userId) })) }),
  ...
  user: router({
    getProfile: publicProcedure.input(z.object({ userId: z.number() })).query(async ({ input, ctx }) => { const [user] = await db.select().from(users).where(eq(users.id, input.userId)).limit(1); if (!user) return null; if (!user.profilePublic && ctx.user?.id !== user.id && ctx.user?.role !== "admin") return { id: user.id, name: user.name, avatarUrl: user.avatarUrl, country: user.country, profilePublic: false }; return user; }),
    ...
    getVideos: publicProcedure.input(z.object({ userId: z.number() })).query(async ({ input, ctx }) => { const [owner] = await db.select({ id: users.id, profilePublic: users.profilePublic }).from(users).where(eq(users.id, input.userId)).limit(1); if (!owner) throw new TRPCError({ code: "NOT_FOUND", message: "Utilisateur introuvable." }); if (!owner.profilePublic && ctx.user?.id !== owner.id && ctx.user?.role !== "admin") return []; return getUserVideos(input.userId); }),
