import { instantWithdrawalRouter } from "./instant-withdrawal-router";
importt { COOKIE_NAME } from "@shared/const";
...
import { videos, likes, comments, followers, earnings, withdrawals, users, notifications, blocks, reports, favorites, shares } from "../drizzle/schema";
    ...
    uploadFile: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          const fileKey = `videos/${ctx.user.id}-${Date.now()}.${fileExt}`;
    ...
    upload: protectedProcedure
      .input(
        z.object({
          ...
          videoUrl: z.string().url(), // Accept URL instead of File
        })
      )
      .mutation(async ({ ctx, input }) => {
        if (!ctx.user) throw new TRPCError({ code: "UNAUTHORIZED" });
    ...
    incrementViews: publicProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          // Create earning: 0.001 USD per view
