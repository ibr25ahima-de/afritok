    ...
    uploadFile: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          console.log(`[Upload] File: ${input.file.name}, Size: ${input.file.size}, Type: ${input.file.type}`);

          const buffer = await input.file.arrayBuffer();
          console.log(`[Upload] Buffer created, size: ${buffer.byteLength}`);
      ...
      .mutation(async ({ ctx, input }) => {
        await db
          ...
          .set({
            ...
            notifyLikes: input.notifyLikes,
            notifyComments: input.notifyComments,
            notifyShares: input.notifyShares,
            notifyMessages: input.notifyMessages,
            notifyPromotions: input.notifyPromotions,
          })
          .where(eq(users.id, ctx.user.id));
