...
export const appRouter = router({
  ...
  video: router({
    feed: publicProcedure.input(z.object({ limit: z.number().int().min(1).max(50).default(20), offset: z.number().int().min(0).max(100000).default(0) })).query(async ({ input, ctx }) => getFeedVideos(input.limit, input.offset, ctx.user?.id ?? null)),
    upload: protectedProcedure.input(z.object({ title: z.string().trim().min(1).max(200), description: z.string().trim().max(5000).optional().nullable(), videoUrl: z.string().url().max(2048), thumbnailUrl: z.string().url().max(2048).optional().nullable(), musicUrl: z.string().url().max(2048).optional().nullable(), musicName: z.string().trim().max(200).optional().nullable(), visibility: z.enum(["public", "followers", "private"]).default("public"), premiumOptions: premiumVideoOptionsSchema.optional() })).mutation(async ({ ctx, input }) => { const [video] = await db.insert(videos).values({ userId: ctx.user.id, title: input.title, description: input.description ?? null, videoUrl: input.videoUrl, thumbnailUrl: input.thumbnailUrl ?? null, musicUrl: input.musicUrl ?? null, musicName: input.musicName ?? null }).returning({ id: videos.id }); if (!video?.id) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Impossible d'enregistrer la vidéo." }); await db.execute(sql`UPDATE "videos" SET "visibility" = ${input.visibility} WHERE "id" = ${video.id}`); if (input.premiumOptions) await applyPremiumVideoOptions(ctx.user.id, video.id, input.premiumOptions); return { success: true, videoId: video.id }; }),
    ...
    incrementViews: protectedProcedure.input(z.object({ videoId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { const video = await getVideoById(input.videoId); if (!video) throw new TRPCError({ code: "NOT_FOUND" }); if (!(await canViewVideo(input.videoId, ctx.user.id, ctx.user.role === "admin"))) throw new TRPCError({ code: "FORBIDDEN", message: "Vous ne pouvez pas voir cette vidéo." }); await db.update(videos).set({ views: sql`COALESCE(${videos.views}, 0) + 1` }).where(eq(videos.id, input.videoId)); return { success: true, views: (video.views || 0) + 1 }; }),
